...
- The performance history file is trimmed to its configured cap on every save; a per-instance counter meant the trim never ran and the file held three times the cap.
- A `--step` debug run is recorded as its own kind of row and left out of the render averages; half the file's rows were two-second single-step runs that dragged every figure down.
