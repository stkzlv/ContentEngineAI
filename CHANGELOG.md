...
- The shipped duck is shallow: **7.7 dB at the default 0.1 / 4**, measured on a real voiceover. `music_volume_db` already puts the music 27 dB under the voiceover, so a deep duck on top of that makes it inaudible rather than unobtrusive.
- Duck depth scales with how far the narration sits above the threshold, so it can only be measured against real speech. The config's tuning table was first measured on a synthetic tone sitting about 21 dB below real narration and understated every row by roughly a factor of two -- it called 0.03 / 8 a 7.6 dB duck where real speech gives 17.5 dB. The table now carries the real figures across seven settings, and the default moved from 0.03 / 8 to 0.1 / 4 to be the shallow duck it was described as.
