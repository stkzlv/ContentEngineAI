...
### Fixed
- `batch.fail_fast` in `config/scraper.yaml` is reachable again. `--fail-fast` is `action="store_true"`, so an omitted flag arrived as `False`, and the loader resolves it with `cli_fail_fast if cli_fail_fast is not None` -- a supplied `False` always beat the configured value. The flag now defaults to `None`. Closes #309.
