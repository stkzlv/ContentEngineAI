...
- Duck depth scales with how far the narration sits above the threshold, so it can only be measured against real speech. The config's tuning table was first measured on a synthetic tone sitting about 21 dB below real narration, which read every row low by roughly 8-10 dB -- it called 0.03 / 8 a 7.6 dB duck where real narration gives 15.1 dB. The default moved from 0.03 / 8 to 0.1 / 4 to be the shallow duck it was described as, on the model as well as in the YAML.
- The table reports attenuation, the ducked music leg against the same leg un-ducked in the same window. An intermediate version compared music during speech against music in a pause, which also contains the music's own level change between those windows: that read 6.8 to 9.8 dB for one setting across the four bundled tracks, where attenuation is stable to about 0.3 dB across the same four.
