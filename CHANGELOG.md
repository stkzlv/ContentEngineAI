...
### Changed
- **Breaking (packaging)**: the project is declared not-installable (`package-mode = false`), the `src` package stanza is dropped, and the release no longer builds or attaches a wheel/sdist. It was only ever run from a checkout via `python -m src.<module>`, and a built wheel installed a top-level `src` package that collides with any other src-layout project. `poetry install` still provides dependencies for the checkout; releases carry notes and the source archive GitHub attaches.
