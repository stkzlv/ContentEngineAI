...
- A technical spec turns that research into eleven planned requirements, each tracked by an issue; the ones that change output ship off by default until the reach test reads out.
- The roadmap gains items for the new work and excludes the reach-test experiments from the 1.0.0 gate.
...
- The promotional, tutorial, audio and subtitle guides now agree with that research: most TikTok users spend time with sound on, cut pace is moderate rather than frantic, closing lines ask genuine questions instead of baiting, and the AI-label and originality rules reflect the 2025-2026 platform changes.
...
- A short signature sting can be mixed at the start of the narration or under its closing line, mastered with the rest of the audio.
...
### Fixed
- The music step can no longer spend minutes on failures: a Freesound OAuth2 refresh is attempted once per render and a failure is one warning naming the remedy that fits (the setup tool for a rejected token) instead of two errors per candidate, each candidate is downloaded once within its timeout instead of three times, and the whole provider chain runs inside a configurable time budget (two minutes by default, under the step warning), after which a local file is used.
...
### Fixed
- Batch cleanup no longer runs the per-product path over the global output directories (state, logs, cache and the rest), which logged a missing-record warning for each of them on every platform; the four scans of the outputs root share one list of those names; the structure validator counts the state directory as expected instead of removable under strict cleanup, and the empty-directory sweep no longer removes an empty reports directory that the validator then reported missing.
...
### Removed
- `LLMSettings` is no longer re-exported from `src.video.config`; import it from `src.ai.llm_settings`, where it has lived since 0.119.1. The re-export was meant to last one release, and it hid four modules that still used the old path. A test now fails on any module that reaches the name through the old package, whether by a direct import, a multi-name import, the submodule path, or an attribute read under a module alias.

## [0.121.3] - 2026-09-14

### Fixed
- A product whose detail page answers the title selector with an empty element is no longer dropped. The page carries `productTitle` twice, once as the span holding the title and once as a hidden input whose text is empty, and the loop broke on the element being present rather than on it having text, so three working fallback selectors were never tried. The product was then discarded as titleless with price, rating and description intact, since validation reduces to a title check under the shipped settings. A batch validated zero of 27 products across two keywords before this. The description loop had the same shape; its last fallback is now scoped to the feature-bullet block, since falling through an empty first match made an unscoped page-wide list selector reachable and that text becomes narration.
...
- The YAML reader for search parameters no longer restates four defaults, and the retired fallback config restated them again; that is where the drifted `sort_order` and `remove_temp_on_success` copies came from. The two layers that remain, the YAML model and the runtime dataclass, are asserted to agree field by field.
...
- A test asserts the import closure in subprocesses, failing if torch, Whisper, Botasaurus, Playwright or the Google Cloud SDK returns to it, and walks every export map, which no other gate reads.

## [0.118.3] - 2026-09-14

### Changed
- Logging calls use lazy `%` formatting everywhere, and ruff's `G` and `LOG` rule groups enforce it with no per-module ignores. 601 call sites carried f-strings, which format the message whether or not the level is enabled and lose the template a log aggregator groups by; the sweep was mechanical and every message is unchanged, with literal percent signs escaped. `logger.error(..., exc_info=True)` becomes `logger.exception(...)` at the 40 sites that used it, and four string concatenations become arguments.
...
### Changed
- **Breaking (packaging)**: the project is declared not-installable (`package-mode = false`), the `src` package stanza is dropped, and the release no longer builds or attaches a wheel/sdist. It was only ever run from a checkout via `python -m src.<module>`, and a built wheel installed a top-level `src` package that collides with any other src-layout project. `poetry install` still provides dependencies for the checkout; releases carry notes and the source archive GitHub attaches. The `make build`/`package` targets and `release-prep`'s build step are removed with it. Tool scripts that import `src` put the repo root on `sys.path` themselves (`performance_report.py` and `cleanup_outputs.py` are invoked as `python tools/<x>.py`, where a script's own directory -- not the repo root -- leads the path; `enumerate_topics.py` runs as a module and carries the same insert defensively), since without the installed package `src` is otherwise unimportable.

## [0.117.1] - 2026-09-13

### Changed
- Function-local imports in the three heaviest modules are hoisted where they were habit and kept where they are load-bearing: stdlib imports move to module scope, cycle-free first-party imports too, and the survivors carry a comment saying what breaks if they are hoisted -- honestly: the publisher SDK and the optional pycaps group are the only imports a function-local placement actually keeps out of the closure, while the scraper and STT/render stacks are already resident via their package __init__ chains and their locals only keep the module headers small. The producer/batch shared names are now module-level, so tests patch them on the consuming module.
...
### Removed
- The Botasaurus-migration-era Makefile targets: one ran a script that does not exist, one offered a hard reset to a commit that is not in the repository, one reported required dependencies as leftovers, one globbed a dead layout, and the retained installer's recipe would have removed two live dependencies. Also gone: the dead mypy override for a module that no longer exists; four config keys nothing reads; and an unreferenced ASS-effect scratch tool.
...
### Fixed
- Overlay text in a script the default font does not carry renders with a covering face instead of empty boxes. Drawtext takes one face and has no fallback chain, so a CJK disclosure override rendered as fourteen notdef rectangles, silently, with FFmpeg exiting 0. All three overlays -- the disclosure, the hook and the upper line -- now resolve a font file through fontconfig that covers every glyph of their text, preferring a sans face; ASCII text keeps the default face with no lookup. Operator-typed text nothing installed can draw is refused with the missing glyphs named when the producer validates its configuration at startup, rather than shipping boxes; the batch path and render time degrade with a warning instead, because losing an unattended render over a decorative overlay would be worse. The query carries every codepoint of the string, not only the non-Latin ones, because the whole string draws with the one chosen face and a face picked for its Devanagari alone would render the Latin half as the very boxes this closes; and it excludes color and bitmap faces, which FFmpeg cannot size at all -- choosing Noto Color Emoji aborts the render outright. Validation reads the merged per-profile settings and checks the custom text only when the effective source is custom. On systems without fontconfig, coverage is unknowable and behavior is unchanged. Closes #392.
...
### Fixed
- Oversized images are downscaled before they enter the assembly filtergraph. FFmpeg buffers decoded frames at source resolution per input stream, so an image-profile assembly over full-resolution stock photos exceeded a six-gigabyte memory cap on the decoder side alone and the kernel killed the encode. Images only ever render inside the output frame, so nothing above the new configurable edge bound ever reached the screen; sources over it get an aspect-preserving temp copy, reused across attempts and swept with the temp directory, while smaller sources, a disabled bound, or a source the resizer cannot read pass through unchanged, because a render that might exceed a cap still beats no render. The copy carries the source's EXIF orientation, since FFmpeg applies the tag to JPEG stills and a copy without it renders a phone photo sideways; its name folds in the source's timestamp and size, so a re-scraped image under the same filename misses the cache instead of serving the previous listing's picture; and it reaches its final name only through an atomic rename, so a kill mid-write cannot leave a truncated copy the reuse check would trust forever. Closes #414.
...
### Fixed
- The limit a step derives for itself can no longer exceed the budget for the whole render. Whisper's timeout is derived from audio length, so with the shipped settings a 59-second voiceover earned it 1008s inside a 900s pipeline: it then finished inside its own budget having spent most of the run's, the pipeline timeout fired during assembly, and the failure was attributed to the pipeline rather than to the step that spent the time. The crossover is a 52-second voiceover, which an ordinary topic script reaches. The remaining budget is now carried to every step that derives a limit, and each attempt is clamped against what is left when it starts rather than when the schedule was built, so a retry cannot be handed time the first attempt has already spent and a timeout names the step that ran out. The final assembly's own limit is bounded the same way. A run with no pipeline timeout is unaffected: no deadline means no outer bound, not no time left. Closes #398.
- A killed encode no longer leaves a file under the finished render's name. Both timeouts recorded on that issue left a non-zero `.mp4` at the path a completed render uses, failing `ffprobe` with `moov atom not found`, which anything checking for existence rather than validity accepts as a finished video. FFmpeg now writes into the run's temp directory and the final name is created only by the rename that follows a clean exit, so the guarantee holds for a cancellation as well as an error. The temp directory rather than a sibling file, because the publisher discovers renders by a glob that a sibling would match, and a kill that skips the cleanup entirely would then leave an unplayable file to publish.
- Both render timeouts raised, and documented in terms of what makes them binding. `pipeline_timeout_sec` goes from 900 to 2700 and `final_assembly_timeout_sec` from 600 to 1800; the pair lost two renders in one day at the old values. What binds them is machine contention and script length rather than hardware class: measured on one machine, an idle render of a 33-second voiceover took 327 seconds end to end, while the same steps under load with a 59-second voiceover took 671 seconds for transcription alone and 543 for assembly. The assembly timeout sits inside the pipeline one, so raising either alone leaves the other binding, which is why both move together. Closes #402.

## [0.108.1] - 2026-09-09

### Fixed
- A reasoning model's chain of thought is no longer accepted as a video description. `validate_description_completeness` checked only that the text was non-empty, at least 50 characters and at least 10 words, and a monologue about writing a description is longer and wordier than a description, so it cleared every floor by a wide margin: one went out as a scheduled post's description on three platforms. Three checks separate them now, each asserting something the prompt already demands. A maximum length, measured rather than chosen: real descriptions from this pipeline run 244-430 characters against a prompt asking for 150-300, while the chain-of-thought outputs ran 1346-2686, so the new `llm_settings.description_validation.max_chars` sits at 900 with room either side. No markdown, which the real output opened with. And a finished last sentence, which it did not have. Hashtags are deliberately not checked, since a monologue omits them as readily as a description does; the sentence check strips a trailing hashtag block first so it does not become a hashtag check by accident, and tolerates a trailing emoji because real descriptions end on one. A rejected description is not fatal: every call site sits inside the generation loop, so the next model or the fallback provider is tried. Closes #404.
- Discovered OpenRouter free models that cannot do this job are dropped before they are tried. The pool is unaudited and changes under the account, and the two models seen in the failing run were a reasoning model and a music model. A model is now skipped when it advertises any output modality besides text, or when its reasoning is on unless a caller turns it off. An unrecognised response shape is kept rather than dropped, because this pool is the fallback that keeps a batch running when the primary provider is rate-limited. Both generators carry their own copy of the two discovery loops and both are filtered.
- Unified captions are clamped against the tightest platform they are sent to, and measured on the caption rather than on the description alone. One caption goes to every platform but was clamped against whichever platform's metadata loaded first, which is YouTube on the bundled config, so captions trimmed to YouTube's 5000-character description limit were posted to Instagram's 2200 and refused. The refusal is total, so a caption too long for the tightest platform lost the post on the platforms it would have fitted; four of five topic publishes were lost this way. The clamp also trimmed `description` while what is sent is the composed caption, which adds the disclosure line, the hashtag block and the blank lines between them, so a description trimmed to exactly the cap still composed past it. Titles had the mirror problem: only YouTube declares a title cap, so a first-loaded TikTok file clamped nothing and left an over-cap title for the YouTube leg. All four publish paths now clamp for the platforms they actually target, including the scheduling path, which did not clamp at all, and the immediate batch, which clamped for whichever platform's metadata had loaded rather than the one it was posting to. The scheduling path has two branches and only one of them posts per platform: its unified branch, which is the default, builds a caption per platform and then sends the first to all of them, so that branch builds its own caption clamped for every target instead of reusing one. It builds a per-platform caption in two places, from a metadata file and from `data.json` when none exists, and both record what they built from, or the rebuild has nothing to work from on a product with no metadata file. The builder's own fallback, taken when the record has no title for a YouTube post, hand-assembles its caption and returned before the clamp, so that path is clamped too. Closes #403.

## [0.108.0] - 2026-09-08

### Added
- `make topics-batch TOPICS=<topics.yaml>` renders a list of topics, one pipeline step per process. `pipeline_timeout_sec` is one budget covering all eight steps of a topic, and a Whisper pass alone can consume most of it (#402). `--topics-file` does not help: it applies that same timeout per record, so every topic in its list has the same problem. Running one step per process is what changes, not the list. It takes the same YAML `--topics-file` accepts and reads it through the project's own loader, so validation, the slug and the product id have one implementation rather than a second one in shell that diverged on accents, on titles past the slug length cap, and on titles that normalise to nothing. Topics are isolated, the summary names the step each failure stopped at, and every result is checked with `ffprobe` rather than by exit code, since a timeout leaves a truncated file under the finished render's name. Assembly's separate `final_assembly_timeout_sec` is untouched and is what to raise if assembly is what times out. Closes #400.
...
### Changed
- `analytics.limit` raised from 50 to 70, in the bundled config and in the code default, because the sweep size and the cadence are coupled: a durability ratio needs a post older than thirty days whose timeline still reaches publication, and the provider retains about five weeks, so the sweep has to cover five weeks at whatever rate the schedule publishes. At two posts a day that is seventy, and leaving it at fifty would have cost every post published from now on its ratio -- silently, since a short sweep looks exactly like a complete one, and unrecoverably, since the rows expire. A test now asserts the two shipped values against each other, so changing the cadence again is free and changing it alone is not.

## [0.106.2] - 2026-09-08

### Fixed
- FFmpeg-engine captions drew two lines on top of each other at nearly every caption change. Both dialogue lines carry the same position, so the frame showed one caption superimposed on the next: the words looked drawn twice and slightly offset, and a sentence boundary read as though its space were missing, because the tail of one caption and the head of the next were on screen together. The timing smoother shifts every word's start 40ms earlier and leaves ends alone, so a caption appears just before it is spoken; Whisper's own word windows are contiguous, and on the measured transcript that shift accounted for every overlap. Nothing then clamped the result, and 36 of 43 boundaries overlapped in the rendered subtitle file. The lead is wanted, so a segment now ends no later than its successor begins rather than the lead being undone -- the same clamp the minimum-duration rule already applied on the one path it covered, and a clamp rather than a cause-side fix because the minimum word duration can produce the same overlap on a short enough word. On the measured render the overlaps go from 36 to none, with every caption and its text unchanged, and a boundary with a real pause keeps its own end. The pycaps engine reads the transcript rather than these segments and is left alone; it showed no such symptom on this script, though that is an absence of report rather than a frame checked at the boundary. Closes #389.

## [0.106.1] - 2026-09-08

### Fixed
- The script fact check listed a repeated claim more than once, spending its repair slots on sentences already named. Seen on a real topic render: the checker returned four claims that were two, each listed twice, so one of the three slots went to a repeat. The repair was still good there, but on a script carrying three genuinely wrong claims a duplicate pushes a real one out of the window with nothing to show it happened. Claims are now deduplicated on both parse paths, keyed on the containment guard's own normalisation, so two spellings differing only in case or punctuation collapse. The relation is not shared: the guard matches by containment either way and this by equality, so a partial quote of the same sentence still takes a slot. Narrower is the safe direction, since it never merges what the guard treats as distinct. The same answer also ran a repeated `VERDICT:` header straight on from the previous `FIX:`, and the parser read the header as part of the correction, so a contaminated string was handed to the reviser as the correct fact. The fix now ends at a verdict header and only at a whole one -- the word, a verdict value, and the end of the line -- so a correction that merely quotes a header, such as "the scanner writes verdict: flagged into the log", is kept whole. Every run-on header observed ends its line, so requiring that costs nothing. Closes #386.

## [0.106.0] - 2026-09-08

### Added
- A static upper line above the visual, holding the affiliate link, the public link-in-bio page, or a fixed line of text. The two-part subtitle system already rendered one, but only under the FFmpeg engine: `step_generate_subtitles` disables two-part mode when the engine is pycaps, so a profile that switched engines lost the line with a single debug line, and pycaps has one caption track and no static element to carry it. The line is static for the whole video and needs no subtitle engine, so it is a `drawtext` overlay beside the disclosure and the hook instead — which means it survives both engines, the pycaps burn that composes over the assembler's output, and the FFmpeg caption fallback, the case where a subtitle-side implementation would lose it exactly when the fallback is doing its job. When it draws a line, two-part's own upper line is turned off so the text is not drawn twice — visibly different rather than identical, since every bundled two-part profile shows its line only during the CTA moments while this one is held for the whole clip, there is no `prefix_replace` equivalent, and the text itself can change, since two-part reads `SUBTITLE_BUSINESS_URL` ahead of any product field while the shipped `source: affiliate_link` substitutes the product's link — `source: link_in_bio` keeps the old address — and it is drawn one size smaller at a fixed height rather than tracking the image; when the source resolves to nothing it is left alone, so a render that would show no overlay keeps the line it had. The voiceover-synced lower half is unaffected throughout. The image is fitted into the rows the line leaves rather than under it, and the caption block is untouched. A source that resolves to nothing renders no line and logs which — a topic has no affiliate link, and an installation that has not set the bio URL has no bio page — rather than drawing an empty box. A line that would render wider than the frame allows is refused for the same reason: drawtext does not wrap and the filter centres the text, so an over-wide line is clipped at both ends. At the default size a shortened link fits and a full tagged `/dp/` URL does not. The bio address is read from `LINK_IN_BIO_URL` and never from the bundled config, which ships no account-specific value; `SUBTITLE_BUSINESS_URL`, which the two-part upper line has read since it shipped, is a fallback, so an installation already showing its bio page needs no second variable. Off by default, overridable per profile. Closes #88.

## [0.105.1] - 2026-09-08

### Fixed
- Topic scripts closed on the honest-limit caveat instead of the result. Each topic template's body asks the script to finish on the result, and its Rules block asked for an honest limit without saying where. On two of the three that bullet sat directly above the call-to-action rule; on `topic_answer_first` a delivery-format bullet sat between, and it produced the caveat close anyway, so proximity is part of the mechanism and not all of it -- a rule stating no position is read as belonging wherever it sits, and the end of a rule list reads as the end. The last spoken line before the call to action was a caveat, which sends the viewer away unsure the fix worked, and the payoff the body asked for was displaced. The limit rule now states its position, sits among the method bullets rather than last, and each body names the result as the line immediately before the call to action. Measured over eighteen scripts per arm, six topics against all three templates: six closed on a caveat before, one after, and that one is a classifier false positive ("You might see a noticeable difference in charging speed" is a result containing the word "might"). Per template, before and after: `topic_answer_first` 3/6 to 0/6, `topic_symptom_cause` 3/6 to 0/6, `topic_mistake_fix` 0/6 to 1/6. The classifier is keyword-based, so it reads the shape rather than the meaning: it counts a debunk close ("If none of that helps, it's probably not the browser itself") as a caveat, and it misses a caveat phrased outside its cue list ("This fix does not apply if your phone's charging circuitry is damaged" scored as a result). The miss understates the baseline, so the measured gap is if anything conservative. Closes #378.
...
### Added
- `script_templates.cta_options` and `cta_options_topic`: the closing lines a script may end on, structured rather than prose, with a separate list for topic renders that have nothing to buy. Every template renders them into a `{CTA_RULE}` -- placed immediately after the closing-beat rule on product templates, and last in the Rules block on topic templates, which state their close in the body -- and the generator refuses a script whose final sentence is not one of them verbatim, re-entering the existing retry loop. If every attempt misses, the first otherwise-complete script gets the first option, with a warning: appended, or replacing its last sentence when that reads as a paraphrased CTA, since two calls to action back to back read wrong and the paraphrase would become the YouTube first comment.

### Notes
- All four attempt paths in script generation -- the primary provider and the fallback one, each with its free-model discovery -- now validate through one closure, so the rule cannot be applied at one site and skipped at another.
...
- The Pydantic default is off, like its sibling overrides: a bare model leaves a template as its author shipped it. Such an install has AI tagging off and a default template pool excluding `explosive`, the only bundled preset that ships a sound effect, so there is nothing there to mute.
...
### Added
- `llm_settings.thinking_budget`, shipped as `0`, applied to every Gemini call the pipeline makes. Gemini's flash tier spends hundreds of thinking tokens on a task whose visible output is a handful, which is where the fortyfold gap between the lite and flash tiers comes from rather than the headline rate. The locked SDK could not turn it off — 1.2.0's `ThinkingConfig` has only `include_thoughts` — though the caret pin itself allowed it: the field landed in 1.10.0.

### Notes
- The pycaps AI word-tagger was passing no generation config at all, and it is the one flash-tier call the pipeline makes: `subtitle_settings.pycaps.llm_model` ships as `gemini-2.5-flash`. Measured at 556 thinking tokens to tag a handful of words, on every render drawing a template with an AI rule — about half of them. It now takes the same budget, measured back to zero.
- Nothing this pipeline asks for is a reasoning problem: a script of about a hundred and twenty words, a caption, a handful of stock-search phrases, a word to highlight. On the lite models the `llm_settings` calls use it changes nothing, since they do no thinking either way; it is also what keeps a later switch to a 2.5 flash model from silently costing more.
- Disabling thinking does not guarantee the model omits a preamble, so the per-generator sanitisers still apply.
- The model stays `gemini-2.5-flash-lite`, measured rather than assumed. Two things came out of the measurement and both argue against moving. Thinking cannot be turned off on the 3.x flash tier at all: `thinking_budget: 0` is ignored, `thinking_level: low` is the floor and still bills, `minimal` is rejected, and `off` is not a value the SDK has — so the fortyfold cost gap stands rather than narrowing to sevenfold. And on six topic scripts run twice each, the stronger model produced fewer phrases (2.3 a run against 3.0) and drifted to literal scene-setting on abstract subjects: for a script about wifi dropping in the evening it spent a phrase on `suburban street evening houses`, which retrieves suburban streets. The lite model stayed on the subject.
...
### Added
- `subtitle_settings.pycaps.ai_tag_prompt_override`: what the AI tagger should highlight. The built-in templates ask for "the most important phrase or word in all the script", and Gemini answers that with `also`, `can`, `all` and `from`, the filler the subtitle best practices say not to emphasise. The bundled config ships a recipe naming what to tag and what never to. Closes #101.
...
- The test that pinned the old value asserted that a profile's `video_aspect_mode` reaches the filter, a plumbing defect that once made every profile letterbox. That assertion is kept by driving the crop request through a CLI override, which merges through the same function, rather than through a bundled profile that no longer crops.
...
### Notes
- The sweep is not gated on a successful publish, and on `schedule` it runs even when there is nothing to schedule: a cron run on a day with no new renders is the day earlier posts fire. The post a run just created is still pending and cannot be judged; the value is in the previous runs' posts. Two runs sweep nothing: `single` on a product already published on every requested platform returns before it builds a publisher, and the global batch skips its publishing phase entirely when nothing was produced.
...
### Fixed
- A no-flag standalone scrape searched the head of the configured keyword list every run. `BatchController._process_keywords` walks the pool from index 0 and stops at `max_products`, so repeated invocations returned the same products. The configured pool is now reordered by date, so consecutive runs start at different keywords. Closes #347.

### Notes
- Only the configured pool rotates. `--keywords` is what the operator typed and stays reproducible, since a date-dependent result is surprising in a tool used interactively to reproduce a problem.
...
### Changed
- Blur-fill backdrops render darker. The default applies to every bundled profile that fills the frame, so their output changes without any config edit; set either field to 1.0 to restore the previous appearance.

### Notes
- Captions sit on that backdrop. Before `blur-fill` the lower frame was solid black, so contrast was fixed; now it varies with the shot. The base caption style is white fill with a black stroke, and the 21:1 `docs/subtitle-best-practices.md` quotes is the fill against its own stroke, which is what keeps captions legible over any backdrop. What a bright one costs is the margin: measured on a real `product_video_primary` render the band ran 102-165 of 255, and white fill against the light end is 2.5:1, leaving the stroke to do all the separating. At 0.6 those three frames read 7.4:1, 10.8:1 and 6.0:1.
...
- Applied at source resolution on the image chain and at 1/6 scale on the video one. `colorlevels` is RGB-only, so leaving it after the scale ran the whole backdrop through RGB at frame size: measured 21s to 139s of filter CPU on a 30s clip.
- The image chain converts the frame before darkening it. `colorlevels` produces striped output from a packed RGBA frame on ffmpeg 8.0.1 and still exits 0, and a PNG with alpha reaches this path from both the scraper and the stock provider. The video chain needs no such conversion because `gblur` runs first and accepts only planar formats, which is why the darkening has to stay after the blur there.
...
### Fixed
- A burn failure left the previous burn's `pycaps_metadata.json` in place. The file is not rerun-blocking, so one written by an earlier successful burn survived and was recorded as the next run's `burn_pycaps_subtitles` artifact, naming a template that render never applied. Every burn failure now clears it. The two exits that skip the burn without failing keep it: a non-pycaps run never attempted a burn, and the record it would delete belongs to another profile of the same product, since the file is product-level and the render is not; and an already-burned run leaves a record that genuinely describes the captions on that file.
...
- Two of the four burn failures can degrade: a pycaps render failure, and pycaps having gone missing since the run that recorded the engine, which a resume reaches because it trusts that recorded decision by design. Both leave a transcript and an assembled video on disk. The other two still abort under `raise` and `fallback_ffmpeg` alike -- a missing transcript leaves nothing to build captions from, and a missing assembled video leaves nothing to burn them onto.
...
- `output_audio_sample_rate` (48000), applied whether or not normalization is on.

### Notes
- Nothing normalized loudness before this: `amix` ran with `normalize=0` and the master level was whatever the fixed track gains summed to. Two real renders measured **-17.4 and -17.6 LUFS** integrated against a -14 target, with true peaks of **-0.1 and -0.2 dBFS**.
- Those two figures together are the argument for R128 over a gain change. The mix was quiet on average *and* nearly touching full scale, so raising the level would clip and limiting alone would leave it quiet.
- Measured after, on real renders of the same product rather than on tones: **-14.9 LUFS** with the ffmpeg subtitle engine and **-15.1 LUFS** with pycaps, both at a true peak of **-0.8 dBFS** and 48 kHz. So the change moves loudness about 2.6 LU toward the target and buys 0.7 dB of peak headroom.
- It does not land exactly on the target, and the reason is the true-peak ceiling rather than the pass being single-pass. The mixed narration arrives at `input_tp: 0.39`, so the gain that would reach -14 LUFS linearly would breach `TP=-1.0`; `loudnorm` therefore refuses linear normalization, falls back to `normalization_type: dynamic`, and reports `target_offset: 1.19` -- it lands 1.19 LU short and says so. A second pass fed only the four `measured_*` values reports the same offset and produces a byte-identical file; the loudnorm author's documented two-pass also feeds back `offset=<target_offset>`, and that does help -- measured -15.2 to **-14.6 LUFS**, about half the shortfall, with the true peak still at -1.0. It is not taken because two-pass needs the mixed audio as a file to measure and that only exists inside the filtergraph, which would mean rendering the audio separately first. A constant tone measures exactly -14.0 because its crest factor never brings the ceiling into play, which is why a synthetic figure should not be quoted as what renders produce.
- The 0.2 dB by which the delivered file exceeds the -1.0 dBTP ceiling is the AAC encode, not the pass: `loudnorm` reports `output_tp: -1.00` and the graph's WAV output measures -1.0 dBFS, while the encoded mp4 measures -0.8. An install that must stay under -1 dBTP in the delivered file should set `loudness_true_peak_db` lower rather than look for something to change in the filter.
- The pycaps burn is not the explanation either. The ffmpeg-engine render never reaches pycaps and is still 0.9 LU short. The two engines differ by 0.2 LU, which is within what a different music draw between two full runs would produce, so that gap is not established as the burn's cost.
- The resample is emitted unconditionally, not as a tail of the `loudnorm` filter. It started life as a fix for `loudnorm` emitting at 192 kHz whatever it is handed (verified), but `output_audio_sample_rate` names an output property, and hanging it off the normalisation branch meant `loudness_normalization_enabled: false` silently dropped the rate control with it. Renders currently come out at 24 kHz (topic, from TTS) and 44.1 kHz (product); without the resample the encoder negotiates a rate nobody chose. This does change the output rate of every render to 48 kHz.
...
- `music_ducking_threshold` is floored at `sidechaincompress`'s own lower bound (0.000977) rather than at zero. A smaller value passed config validation and then aborted the render at final assembly, after the script, voiceover and subtitles had been paid for -- and the tuning table below points in that direction.
- The shipped duck is shallow: **5.3 dB of attenuation at the default 0.1 / 4**, measured on a real voiceover. `music_volume_db` already puts the music 27 dB under the voiceover, so a deep duck on top of that makes it inaudible rather than unobtrusive.
- Duck depth scales with how far the narration sits above the threshold, so it can only be measured against real speech. The config's tuning table was first measured on a synthetic tone sitting about 21 dB below real narration, which read every row low by roughly 7-8 dB (it also used the window method below, so the two errors compound) -- it called 0.03 / 8 a 7.6 dB duck where real narration gives 15.1 dB. The default moved from 0.03 / 8 to 0.1 / 4 to be the shallow duck it was described as, on the model as well as in the YAML.
- The table reports attenuation, the ducked music leg against the same leg un-ducked in the same window. An intermediate version compared music during speech against music in a pause, which also contains the music's own level change between those windows: so it varies with the music as well as the duck. Single-window attenuation still varies across the three bundled tracks -- 5.3 / 5.6 / 6.4 dB at the default -- but that variation is the music's own dynamics rather than a different quantity being measured, so the table names the track it was measured on.
...
- `apply_aspect_ratio_mode` generates a valid label when the caller omits `output_label`. The old default was `f"{input_label}_scaled"`, giving `[0:v]_scaled` -- brackets in the middle and a `:` FFmpeg reads as an argument separator. Every mode returns that label and the caller appends it, so every mode was rejected the same way (`Trailing garbage after a filter`); blur-fill additionally derived four internal labels from it. No production call site omits the argument, so this was reachable only from a direct call.
...
### Fixed
- The stock-search prompt required every phrase to name a person or a place, which is what produced `hand holding compressed air` and `person unscrewing laptop bottom`. Phrases now spend their words qualifying the object instead. Refs #307.

### Notes
- The failure was in the phrases, not in retrieval. #307 attributes the mismatched shots to the library answering a good phrase loosely and proposes filtering the candidate pool; measured against the live API, rewriting the phrase fixes the same query with no code change. `hand holding compressed air` returns a hand crushing a plastic bottle; `canned air duster` returns someone cleaning a PC case with an electric duster.
- The budget is what made the old rule harmful. A phrase is `MIN_PHRASE_WORDS` to `max_words_per_phrase` words -- three to five -- so `hand holding` spent two of them on words that appear in a large share of the library's captions. That is why a query for compressed air returned a hand holding a hotdog: it matched the hand.
- Mechanically dropping the human words is *not* the rule, and the prompt does not say so. `hand plugging in usb cable` returns a USB going into a laptop port and is correct as written; stripped to `plugging usb cable` it returns a phone charger. Scaffolding is harmless when the object is already specific, so the rule targets the budget, not the words.
- The old rule was defending something real. Deleting the prompt's examples once produced bare categories -- `wifi router` in 3 of 10 runs -- and a two-word phrase is dropped by the sanitiser, after which the render falls back to a single generic search for the whole video. The both-halves rule reached the floor by attaching a person. The replacement reaches it with qualifiers, and says outright that a short phrase is discarded rather than merely weak.
- Measured on one topic script, same script both ways: 1 of 6 returned shots matched its query before, 4 of 6 after. Small sample, and one miss remains -- `compressed air can spray` returns spray paint, because `spray` names a different product.
- The regression the old rule guarded against does not reappear. Across 10 generations on two scripts, every phrase reached the word floor: none was dropped by the sanitiser and none carried a generic person or hand. Ten clean runs would occur about 3% of the time if the bare-category failure were still at its measured 3-in-10 rate.
- Selecting better from the returned pool was implemented and thrown away first. Word-overlap scoring, rarity-weighted scoring and provider rank each put a decorative paper fan at or near the top for `laptop fan always on`; string matching cannot tell it from a cooling fan. The evidence is recorded on #307 so it is not rebuilt.

## [0.89.1] - 2026-08-30

### Fixed
- A profile's `video_aspect_mode`, `video_assembly_mode` and `first_frame_pre_motion` were ignored on every render. `VisualFilterBuilder` bound `video_settings` to `self.config.video_settings`, so each of those reads took the global value. Three bundled profiles override the first two and one overrides the third. The same binding also moves `transition_duration_sec`, `enable_format_normalization` and `pre_motion_peak_zoom` to the profile value; no bundled profile sets them differently, but a fork that does gets a behaviour change.
- The caption layout and the assembler disagreed about `transition_duration_sec`. `subtitle_builder` computed segment boundaries from the global while the assembler laid out the crossfades from the merged value, so a profile changing it drifted every content-aware caption reposition after the first visual, cumulatively. That is the ffmpeg subtitle engine only: the bundled config selects pycaps, which emits no SRT or ASS and never reaches these call sites. On ffmpeg it affects the main narration line on every profile, and the upper URL line where two-part is enabled. `_calculate_segment_times` has four callers, not one. Both read the merged value now. Latent until the fix above made the assembler side honour the profile.
...
- Two tests pin it, and the first attempt at them did not. A test that constructs the builder with no profile and passes the aspect mode in as a literal exercises the filter emitter, which was never broken -- nine of ten assertions passed against the unfixed code. What catches it is driving `build_visual_chain` under a real profile and asserting `crop=` appears, plus an AST check derived from the `_collect_overrides` target set asserting no overridable field is read through the global inside that method. The AST check resolves aliases two hops deep (`cfg = self.config` reaches the object as surely as binding it directly), treats `getattr(obj, "field")` as an attribute read, and exempts a global read only when the same field is read off the profile in the other operand of the comparison -- exempting every operand of every comparison let a defect hide inside any `if a != b`. Each of those was a shape that defeated an earlier version with the suite green.
...
### Notes
- The first one is not an edge case. Ten of the eleven bundled profiles take that arm, including all nine a `--random-profile` run can draw, so every fresh *product* render failed at `gather_visuals` from v0.82.0 to v0.88.0. Topic renders were unaffected: they run on `slideshow_stock`, the one profile that fetches stock media and therefore the one that took the other arm. Found by the post-release end-to-end batch; the suite never drove the step, and its short-circuit on an existing `gathered_visuals.json` hid it on re-runs.
...
- The guard reads CPython's own definite-assignment analysis rather than reimplementing it: the compiler emits `LOAD_FAST_CHECK` where it cannot prove a plain local is bound. It covers every function in the module, which is how the second bug was found, and needs no maintaining as the syntax grows.
- It is not total, and both gaps were measured rather than assumed. A conditionally bound name captured by a nested function is read with `LOAD_DEREF` and is not seen. A comprehension target read after the comprehension is narrower than it looks: that read compiles to `LOAD_GLOBAL`, so it is missed only while the name is bound nowhere else in the function -- a real local of the same spelling turns it into `LOAD_FAST_CHECK` and it is caught -- and being a global lookup it fails only when no module-level name shares the spelling. Tests assert this module carries neither shape, by the property rather than by the name: pinning that one function has cell variables stays green when its binding moves under a condition, which is the crash it exists to prevent.
- A hand-rolled AST walk was written first and discarded. It reported names on legitimate code across the producer package, and was blind to bindings in `try` bodies, `except` handlers and `with ... as`. It also passed against the reverted fix, because the step's body sits inside an `async with` it did not descend into.
...
### Removed
- **Breaking, config:** `video_audio_handling` and `video_original_volume`, and the uncalled `AudioFilterBuilder.build_audio_filters_with_video_audio` they fed. Source video audio is not carried into the render, which is what already happened. Closes #330.

### Notes
- The method was defined and never called: the assembler's only audio call site is `build_audio_filters`, whose signature takes no video-audio arguments. So three bundled profiles configured a mix and got silence, and `grep video_audio_handling src/` found a file that looked like a reader while every occurrence sat inside the unreachable method.
- Wiring it up was the other option, and the profile comments assumed it ("Include video audio at reduced volume"). Three things argued the other way: it has never run in any release, so nothing regresses; at the configured -30/-35 dB it sat below the music bed at -24 dB under narration at +3 dB, so the benefit was inaudible; and both video sources are third-party, so mixing either into an upload is an audio-match risk on all three platforms, where a claim or a mute zeroes the video.
- Removing the fields makes them loud. `VideoProfile` sets `extra="forbid"`, so a fork whose profile names one now aborts config load instead of having the value accepted into a merged setting nothing reads. That is the whole of the breakage, and it is why this is a minor rather than the patch the removed-code-had-no-caller framing would suggest.
- `KNOWN_DEAD_TARGETS` in the profile-override audit is now empty, and the test asserts that rather than asserting it only shrinks. The old check required every recorded name to still be mapped, which passes for any name a future author would plausibly add -- a dead target is by definition a mapped one -- so the guard meant to stop the set growing could not.
