...
- The standalone pagination stopped for a dead query but not for one whose retries were spent, so a throttled keyword still opened a browser session per remaining page — straight back into the block the backoff exists to let cool. It reads a give-up signal rather than the summary list, which an input joins on its first retry and would have ended it a page early.
- Telling Botasaurus to let the error page through also made it write an Amazon page and a screenshot into the working directory on every raise, with nothing pruning them across runs. Disabled, as the downloader already does.
