...
### Notes
- The provider joins a keyword list into one query string, so several phrases passed together collapse onto whichever one dominates: the library returns a full page of results that all match a single phrase and ignore the rest. Separate searches are what make more than one phrase useful.
- Truncating a resumed run's state now deletes the outputs of the steps it drops. Dropping a step from the state alone did not make it re-run: `gather_visuals` short-circuits on its artifact file existing, so a lost script was replaced by fresh narration and then paired with the footage searched from the old one.
