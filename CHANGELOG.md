...
### Changed
- Every script template in `src/ai/prompts/scripts/` (15 files) now requires a natural conversational hook in the first spoken line that carries the long-tail audio keyword (product category, problem or benefit, audience or use context, price band when supported) embedded inside speech a person would actually say out loud. The rule provides four good shapes (problem-first, direct address, discovery framing, myth-tension) and one explicit anti-pattern (the literal Google-query shape "Best [X] under $[N] for [Y]"). TikTok transcribes spoken audio as a primary search signal, so the keyword's presence in the first 5 seconds of TTS determines whether the video gets discovered; the keyword must be in the audio without the opener reading as a search-bar query.
