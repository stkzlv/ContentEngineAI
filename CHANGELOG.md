...
- The batch's slot search treats slots held in the local schedule as occupied, as `single` and `schedule` already did, so a slot recorded locally but not yet visible on the provider is not offered twice.
...
- The occupancy read that `schedule`, `single` and the batch share survives any provider error or a malformed timestamp, not only a publish failure, and the run keeps scheduling with the slots it could read; a credential rejected on that read used to abort `schedule` outright.
