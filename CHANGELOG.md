...
### Notes
- A durability ratio is reported as unmeasurable rather than negative when the lifetime total reads below the day-30 figure. That cannot happen under a correct cumulative reading, so the number would be an artifact rather than a measurement. Not observed against the live API across twelve multi-platform posts; guarded because absence is this project's answer to an unanswerable figure.
