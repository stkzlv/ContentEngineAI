...
- It is not total, and both gaps were measured rather than assumed: a conditionally bound name captured by a nested function is read with `LOAD_DEREF`, and a comprehension target read after the comprehension with a plain `LOAD_FAST`. Both raise at runtime and neither is seen. Tests pin both, and assert this module contains no such shape.
