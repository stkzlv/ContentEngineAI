...
- A `config/publisher.yaml` in the working directory is no longer read at all; the batch takes the one shipped with the package, which is what the publisher CLI has always done. An install that kept a per-directory copy has to move it.
