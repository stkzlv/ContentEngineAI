...
### Notes
- The rebuild guard fires when the file holds rows and none of them loaded, not when the result is empty. Two earlier versions were wrong in different ways: one counted rows that loaded, which is zero in exactly the case worth guarding; the next required an empty result, which a handful of surviving product directories defeats — three scanned rows would have been written over 323.
