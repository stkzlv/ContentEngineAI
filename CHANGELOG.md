...
### Fixed
- A post scheduled through `single` is written to the local schedule, so `calendar` lists it. Only the batch scheduler wrote that file, so the newest local entry was from June while the account had published daily since, and `calendar` answered "nothing scheduled" with a week queued on the provider.
