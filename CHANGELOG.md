...
### Added
- Hook overlay renders a distinct authored headline (a short LLM-generated line, keyword front-loaded, 3-7 words) instead of repeating the script's first spoken sentence that the running captions already show. Removes the duplicated-text look where the same words appeared as both the top hook and the bottom captions. Generated in the script step from a new `hook_headline` prompt; falls back to first-sentence extraction when no headline is available, so behavior is unchanged for forks without the prompt.
...
- Analytical script templates no longer fabricate a spec the product doesn't have. The rule's spec branch demonstrated itself with a worked example about ports, and on a Bluetooth tracker tag the model reproduced that example's subject almost verbatim for a product with no ports. The spec branch now ships no closing-line example to copy, and its self-check requires quoting a measurement verbatim from the description with its unit as a whole word, so a unit found inside a longer word ("supports" is not ports) no longer counts as a match.
