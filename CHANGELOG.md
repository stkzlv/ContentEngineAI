...
- The doc-citation sweep reads `docs/` recursively, so the moved notes are checked for the first time: three citations to source line numbers now name a symbol, and a placeholder filename is written as one. Cross-references that pointed within `CLAUDE.md`, in the notes and from five test docstrings, name the notes file that carries the text.
