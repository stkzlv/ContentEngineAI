...
- Three separate places swallowed the error page after it was detected: the keyword arm's navigation handler, the standalone scraper's browser wrapper, and Botasaurus's own `@browser` decorator, which catches everything the decorated function raises, re-runs the whole task four times and returns `None`. The caller then saw no exception and recorded the throttled input as a success, after paying for four more browser launches into a live block. The error page has its own exception type now, named in `must_raise_exceptions`, which is the only thing that decorator honours.
