...
- It does not land exactly on the target, and the difference is not the pycaps burn -- that accounts for 0.2 LU of it. Single-pass `loudnorm` is adaptive, and on real narration it settles about 1 LU short and 0.2 dB over the requested true-peak ceiling. A constant tone through the same chain measures exactly -14.0, which is why the synthetic figure should not be quoted as what renders produce.
