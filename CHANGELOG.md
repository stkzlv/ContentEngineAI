...
- Applied at source resolution on the image chain and at 1/6 scale on the video one, with a `format=` on each side. `colorlevels` is RGB-only, so leaving it after the scale ran the whole backdrop through RGB at frame size: measured 21s to 139s of filter CPU on a 30s clip. The conversion ahead of it is equally load-bearing, since the filter produces striped output on a packed RGBA frame and still exits 0, and a PNG with alpha reaches this path from both the scraper and the stock provider.
