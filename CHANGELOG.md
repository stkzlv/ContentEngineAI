...
- Four shipped documents, `config/subtitles.yaml` among them, told the reader to write a flat key per profile. Following any of them now aborts every render rather than the one profile being edited, since the loader raises on the first offender. A test derives the sweep from the legacy maps rather than from a list of files, so it cannot go stale when a key is added. It covers every legacy name except `subtitle_engine` and `subtitle_format`, the only two that are also real global keys and so cannot be judged from the token alone.
