...
- The length floor is enforced in `sanitize_visual_search_phrases`, not only stated in the prompt, and rendered into the prompt from the same constant. Stated separately the two drifted: the prompt asked for three words while the filter accepted two, so a bare object name survived on the model's say-so. `max_words_per_phrase` now floors at 4, since a configured maximum below the minimum renders a prompt asking for "3 to 2 words" and drops every phrase that obeys it -- silently, falling the render back to a title-only search.
