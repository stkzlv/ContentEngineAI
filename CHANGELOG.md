...
- The producer records the pillar it resolved in `pipeline_state.json`, not only one passed as a CLI override. The registry reads that file, so a standalone render of a scraped product would otherwise have been filed unlabelled while its script was written under the pillar. It is resolved after the state load rather than inside the script step, because a resume that truncates the state drops the key and then skips the step that would rewrite it. The registry falls back to `data.json` when the state file is absent, which is the normal case: a successful non-debug render deletes the `temp/` directory the state file lives in, and the registry is written afterwards.
