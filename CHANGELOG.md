...
### Added
- `llm_settings.thinking_budget`, shipped as `0`. Gemini's flash tier spends around a thousand thinking tokens on a task whose visible output is sixteen, which is where the fortyfold gap between the lite and flash tiers comes from rather than the headline rate. The 1.x SDK could not turn it off: its `ThinkingConfig` exposed only `include_thoughts`, so the control was unreachable behind the caret pin.

### Notes
- Nothing this pipeline asks for is a reasoning problem: a script of about a hundred and twenty words, a caption, a handful of stock-search phrases. The configured lite models do no thinking anyway, so this changes nothing today; it is what keeps a later switch to a flash model from silently costing forty times more.
- Disabling thinking does not guarantee the model omits a preamble, so the per-generator sanitisers still apply.
- The model stays `gemini-2.5-flash-lite`, measured rather than assumed. Two things came out of the measurement and both argue against moving. Thinking cannot be turned off on the 3.x flash tier at all: `thinking_budget: 0` is ignored, `thinking_level: low` is the floor and still bills, and `minimal` and `off` are rejected — so the fortyfold cost gap stands rather than narrowing to sevenfold. And on six topic scripts run twice each, the stronger model produced fewer phrases (2.3 a run against 3.0) and drifted to literal scene-setting on abstract subjects: for a script about wifi dropping in the evening it spent a phrase on `suburban street evening houses`, which retrieves suburban streets. The lite model stayed on the subject.
