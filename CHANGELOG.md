...
- Persistent on-frame disclosure overlay (`#ad` by default, configurable text). Burned in a fixed corner of every produced video, full-clip duration, sized at 55% of the subtitle font per FTC guidance. Configurable via `video_settings.disclosure_overlay` in `config/video_production.yaml`: enabled/disabled, text, position (top-left / top-right / bottom-left / bottom-right), size factor, font color, outline, semi-transparent background box, edge margins. Survives the pycaps subtitle pass because pycaps overlays its captions on the assembler's output. New `src/video/assembler/overlay_builder.py` module produces the FFmpeg drawtext filter; injected as the final video filter in `assembler.core.assemble_video`. Defaults clear the YouTube Shorts top header (~10%) and the TikTok bottom username block (~12%).
