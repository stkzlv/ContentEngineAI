...
- Per-pillar audience override under `script_templates.pillar_audiences`. When a pillar is set, `{AUDIENCE}` substitutes the pillar's audience hint (value = budget-conscious shoppers, novelty = curious early discoverers, utility = practical problem-solvers) instead of the global `target_audience`. Templates speak to each pillar's actual buyer.
...
- `generate_script` logs an info-level hint when `--pillar` is set but the value isn't configured in any of `pillars`, `pillar_preambles`, or `pillar_audiences`. Catches typos like `--pillar utillity`; the run is otherwise unchanged (template filter, preamble, and audience override all gracefully no-op for unknown pillars).
