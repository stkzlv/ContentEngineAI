...
- Four shipped documents, `config/subtitles.yaml` among them, told the reader to write a flat key per profile. Following any of them now aborts every render rather than the one profile being edited, since the loader raises on the first offender. A test derives the sweep from the legacy maps rather than from a list of files, so it cannot go stale when a key is added. It cannot judge a name that is also a legal key somewhere else -- `subtitle_engine` and `subtitle_format` globally, `pycaps_template` and its two siblings under `global_batch`, and `two_part_subtitles`, which is the nested block's own name -- so it is a backstop for review, not a replacement.
