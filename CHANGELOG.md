...
### Added
- Phase 1.3 short profile and per-platform routing. New `slideshow_short_20s` profile in `config/video_production.yaml` with a ~50-60 word script budget at 150-180 wpm TTS pacing, sized for the 15-30s hook-iteration zone. A new `profiles: <platform>: <profile_name>` block in `config/publisher.yaml` maps each platform to a profile, and `src/publisher/video_selector.py::select_video_for_platform` prefers `video_<asin>_<profile>.mp4` per platform with a fallback to the first matching render. `PublisherConfig.profiles` accepts the mapping; unset / empty preserves the pre-1.3 first-match behaviour. Closes the Phase 1.3 work in `docs/roadmap.md`.
- Phase 1.2b pre-motion (Ken Burns settle-zoom) on the first image segment. Frame 0 sits at `pre_motion_peak_zoom` (default 1.10); the zoom decreases by `(peak - 1.0) / total_frames` per frame, landing at 1.0 on the last frame. Centre stays fixed. New fields on `VideoSettings` and matching profile-level overrides on `VideoProfile`. Enabled by default on `slideshow_short_20s`; opt-in on the existing 30-45s profiles. Defeats the static-still-then-fade pattern that burns the 1.5-second decision window.
- Phase 1.2c burned-in hook overlay (closes Issue #102). `HookOverlaySettings` on `VideoSettings` plus `apply_hook_overlay` / `extract_hook_line` / `build_hook_drawtext` in `src/video/assembler/overlay_builder.py`. The overlay renders the first sentence of the spoken script as a centre-upper static text on the first 1.5s, 1.35x narration captions, max 7 words, no per-word reveal. Drawn after subtitles and before the disclosure rewrite so the corner `#ad` stays on top in the z-order. Source text is `ctx.run_paths["script_file"]`; empty/missing script makes the overlay a no-op.
- Phase 1.2e cold-open variant rotation framework. `cold_open_variant_pool` on `VideoSettings` with three named variants (`mid_zoom_title_card`, `static_title_card`, `pre_motion_only`) and a deterministic salted-MD5 selector keyed on the product ID (`src/video/cold_open_selector.py`). `step_assemble_video` picks one per render and persists the name in `pipeline_state.json` so downstream analytics can segment retention by variant. v1 ships the framework only — all three variants render identically; variant-specific visual differences follow once a baseline render exists to A/B against.
- Phase 1.2f hook-line lead in the subtitle timing smoother. The first N words get an extra `hook_lead_sec` shift on top of the base `lead_sec` so sound-off viewers parse the opening hook before any audio cue. New `hook_lead_sec` (default 0.20) and `hook_lead_word_count` (default 3) fields in `subtitle_settings.timing_smoothing`. Set `hook_lead_sec: 0.0` to disable. Pairs with the Phase 1.2c overlay so the burned-in title card and the first karaoke word both lead the audio by similar amounts.

### Changed
- Phase 1.2a anti-setup clause added to all 15 script templates' `## Rules` block. Line 1 must state a concrete fact, result, or observation about the product, not a "Today I'll", "Let me tell you", "In this video", or "I want to share" framing. Body framings per template are left alone — the Rules-block clause enforces payoff-shape on top of each archetype, matching the body-vs-Rules pattern in `CLAUDE.md`. A new parametrized sanity suite at `tests/prompts/test_script_templates.py` asserts the 1.1 / 1.2 / 1.5 cross-template rules are present in every template.
- Phase 1.5 closing-line rule on the 8 analytical templates (`problem_solution`, `myth_buster`, `comparison`, `before_after`, `challenge_dare`, `classic_promo`, `rapid_fire`, `question_driven`) now branches on whether the product has a contestable performance number in its description. Spec-rich products (W, mAh, Hz, GHz, MP, GB, ports, hours of battery, dB, Mbps, lumens, Nm, PSI, ANC, refresh rate) still close with a spec claim. Passive products (mounts, hooks, organizers, brackets, kitchen tools, decor, manual gadgets) close with a material-or-use claim instead. Fixes a fabrication bug where the LLM invented numeric specs for products that have none ("eight hours of battery life is the sweet spot for phone holders") and then walked the claim back in the same sentence. Branch condition is a mechanical keyword self-check, not a judgement call, so the LLM picks reliably.

### Documentation
- `docs/subtitle-best-practices.md` §5 splits the audio-lead rule into a narration lead (40 ms) and a hook-line lead (100-300 ms over the first 3 words). §4 gains the conservative single-rectangle cross-platform fallback (centre 888×1160 px on 1080×1920) for templates that ship one design for all three platforms.
- `docs/promotional-video-best-practices.md` §1 splits the hook into a static title-card shape (1.0-1.5s, hard cut to motion) and a text-over-mid-action-frame shape (1.5-3.0s). Adds the 0.3-0.5s pre-motion guidance for static product photos and the 2-3 cold-open variants per pillar requirement. §3 clarifies the corner conflict between the centre-upper hook overlay and the top-left/right `#ad` disclosure. New §6 covers AI-content disclosure (YouTube's July 2025 monetization revocation, TikTok C2PA auto-flagging); the existing "Honest gaps" section is renumbered to §7.
- `docs/roadmap.md` and `docs/roadmap.private.md` Phase 1.2 entries rewrite to reference the title-card / text-over-frame split, the pre-motion rule, and the cold-open variant rotation. Phase 1.3 notes the TikTok CRP 60s+ eligibility constraint (Phase 6.1 long-form profile carries that) and pins the per-platform routing field shape.
