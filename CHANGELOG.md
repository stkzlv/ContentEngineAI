...
- A `publisher.yaml` the loader cannot read now stops the batch instead of being replaced by defaults. That covers a file that will not parse as well as a value it refuses; both used to end at the same defaults, which publish immediately, to the default platforms, with cleanup on. Previously an unusable value aborted only the publishing phase, after the scrape and the renders were paid for. An absent provider credential is the exception: the batch reads settings from this file and takes its key from the environment, so it keeps the configured settings and defers the credential check to where it already happened.
