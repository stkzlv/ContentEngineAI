...
class BotasaurusAmazonScraper(BaseScraper):
    ...
    def __init__(
        ...
        if self.debug_mode:
            ...
            self.logger.info("⚙️ Browser config: %s", self.browser_config)
        else:
    ...
    def _scrape_single_pass(
        ...
        try:
            ...
            if self.debug_mode:
                self.logger.debug(
                    "🔧 [DEBUG] Creating dynamic browser function with "
                    "DEBUG_MODE=%s",
                    self.debug_mode,
                )
            ...
            try:
                ...
                if self.debug_mode:
                    ...
                    self.logger.error(
                        "❌ [DEBUG] Traceback: %s", traceback.format_exc()
                    )
                raise
