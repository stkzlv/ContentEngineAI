...
- **Affiliate literal phrase**: Configured via `config/publisher.yaml::affiliate_disclosure`. When enabled, the phrase is injected into every post's caption between the `#ad` line and the description. Supports non-Amazon programs via `program` key. The closing-frame overlay is deferred.
