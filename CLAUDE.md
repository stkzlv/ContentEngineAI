...
- **Template families share one directory and the default pool is a glob over it.** `script_templates.topic_templates` names the topic family, and `select_script_template` excludes it from the product pool as well as narrowing to it for topics. Adding a template without categorising it makes it selectable by both, and `tests/prompts/test_script_templates.py::test_every_template_is_categorised` is what catches that.
