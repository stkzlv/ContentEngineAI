...
- **Registry pillar source is `pipeline_state.json`, not `data.json`**: `_read_pillar_from_state` in `product_registry.py` reads from the producer's state file because that's what the producer actually used. `data.json` carries the pillar again since #239, but the state file remains the right source: the registry wants the pillar the video was actually rendered under, where a CLI override wins over the product-level one.
