...
- **SDK `posts.get`/`posts.list` crash on an in-flight leg**: while a platform leg is `processing` (e.g. just after a `posts.retry`), its `platformPostUrl` is an empty string `""`, and the SDK's strict Pydantic model rejects it (`platformPostUrl Input should be a valid URL, input is empty`), raising before you can read status. `None` URLs validate fine; `""` doesn't. To poll a post that's mid-retry, bypass the SDK and hit the raw REST API: `GET https://zernio.com/api/v1/posts/<id>` with `Authorization: Bearer $LATE_API_KEY`, then read `.post.status` and `.post.platforms[*].status`. For a `processing` leg the empty URL clears once the leg settles (~1 min). But a **`published` TikTok leg can keep `platformPostUrl: ""` indefinitely** (TikTok often returns no post URL), so the breakage is persistent, not transient: once such a post lands on page 1 of `posts.list`, EVERY subsequent `list`/`get` raises. Impact is path-dependent: `single`/`schedule` catch it and continue with degraded slot-occupancy + blob-retention (API-side posts invisible, fall back to local `schedule.json`), but `verify-comments` lets the `PublishError` propagate and **hard-fails (exit 2)** — the first-comment monitor is bricked while any such post exists. Workarounds: scan/poll via the raw REST list (`GET /api/v1/posts?page=N&limit=50`), and check first-comment delivery via the comments inbox (`pub.get_post_comments(platformPostId, accountId)`), which doesn't go through `posts.list`/`get`.
