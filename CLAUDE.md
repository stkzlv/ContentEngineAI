...
- **Jamendo CDN is slow without proxy**: even with HTTP/2 via curl, the Jamendo CDN is extremely slow from this network (~90 B/s, 3.3 MB tracks time out in 180s). Through the sing-box proxy (HTTP proxy on `127.0.0.1:2080`), downloads complete in under 120s. The provider chain falls through to Freesound if Jamendo times out. If a run is missing background music, the proxy likely needs to be enabled.
