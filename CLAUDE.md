...
- **`BaseProductData.to_dict()` is the single serialiser for `data.json`.** Both paths go through it: the topic path directly, and the scraper via `_product_to_dict`, which now just delegates. A field added to the dataclass and to `to_dict()` reaches the file whichever path wrote it, and `tests/scraper/test_data_json_serialiser.py` fails if a declared field stops being serialised. It compares `to_dict()` against `dataclasses.fields()` rather than against a second dict, because two serialisers can agree with each other and both be missing a field — which is exactly how `pillar` used to persist for topics and vanish for scraped products. `to_dict()` routes both enums through `_enum_value`, since `ProductData(**loaded_json)` does no coercion and a record read back from disk carries plain strings where `.value` would raise.
