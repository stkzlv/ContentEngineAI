...
- **Registry pillar source is `pipeline_state.json`, not `data.json`**: `_read_pillar_from_state` in `product_registry.py` reads from the producer's state file because that's what the producer actually used. `_read_pillar_from_state` tries the state file first and falls back to `data.json`. The state file is preferred because it records the run's own decision, so a `--pillar` override wins; but it is usually gone by the time the registry runs, since a successful non-debug render deletes the `temp/` directory holding it and the registry is written after that. Reading only the state file filed every non-debug render unlabelled.
