...
- **`registry --rebuild` is destructive**: it scans local `data.json` files and overwrites the registry without merging. When product dirs have been cleaned up after publishing, the rebuild produces fewer entries and silently wipes the rest. Always check the `.bak` file exists before running. Tracked as #137 (backup-before-overwrite + merge-not-replace).
- **`record_publish` may not fire for scheduled posts**: six successful publishes on 2026-05-27 produced zero entries in `publish_history.json`. The code path in `cli.py:349` looks correct but the tracking write didn't happen. Tracked as #138. Until fixed, verify `publish_history.json` after each publish session.
