...
- **`--strict` exists on all three CLIs and means the same thing**: exit non-zero when any product failed, not only when none succeeded. `PipelineSummary.exit_code(strict=...)` is the batch's; the scraper and the producer implement it separately. It counts *failures*, not skips: a product rejected for insufficient media is a different category throughout this codebase and does not trip it. On the scraper it had to read a new `BatchSummary.failed_keywords`, because `BatchController._process_keywords` records no per-product result for a keyword that returns nothing or whose search raises — every `ProductResult` that arm produces has `success=True`, so a check reading `summary.failed` alone was a no-op on exactly the runs the docs use as examples.
