...
- **The timeline has a retention horizon of about five weeks, and `from_date` does not widen it.** Measured live on 2026-08-23: posts aged 121, 157 and 188 days all returned rows starting 2026-07-16/17, identical with and without `from_date`; a post 248 days old returned none at all. So **day-2 and day-7 must be captured before a post ages out** — run `analytics` on a schedule, not once at the end of a comparison, because those figures cannot be recovered afterwards. `save_metrics` merges per field for the same reason: a truncated re-read carries a `views_total` counted from mid-life and nothing else, and taking it wholesale erased figures captured while they were still reachable. View counters keep the larger value, since they only accumulate.
