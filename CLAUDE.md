...
- **Re-running a step drops the recorded steps that read its output** (`_drop_dependents`). `--step assemble_video` used to leave `burn_pycaps_subtitles` marked done over a video whose captions had just been re-rendered away, and the next full run skipped the burn and reported an uncaptioned video as complete. The dependents come from the same `step_dependencies` map the executor uses, imported lazily because `orchestration` imports `state`. It also routes them through `_discard_stale_artifacts`: dropping the state alone is not enough, because `generate_description` and `create_voiceover` short-circuit on their own output files, so `--step generate_script` would otherwise leave the superseded script's captions and narration in the render while reporting every step complete.
