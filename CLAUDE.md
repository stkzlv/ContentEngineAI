...
- **Never pipe a gate into `tail`/`head` when chaining with `&&`.** A pipeline's exit status is the *last* command's, so `poetry run mypy . 2>&1 | tail -1 && poetry run pytest` runs pytest even when mypy failed, and the chain reports success. This has shipped a red CI twice: once hiding a `ruff format --check` failure behind an earlier "All checks passed!", once hiding two mypy errors in a new test file. Run each gate bare, or filter with `grep -E "error:|Success"`, which shows the failure instead of the last line of a traceback. `set -o pipefail` also works but does not survive a fresh non-interactive shell.
