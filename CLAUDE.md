...
- **Registry pillar source is `pipeline_state.json` first, then the metadata files at the product root — never `data.json`.** `_read_pillar_from_state` in `product_registry.py` wants the pillar the video was *rendered* under, which `--pillar` can make different from the scraped one. The state file is the run's own record, but it is usually gone by the time the registry runs: a successful non-debug render deletes the `temp/` directory holding it, and the registry is written after that. The metadata files sit at the product root, survive cleanup, and carry the resolved pillar for exactly this reason: the unified `metadata.json`, or the per-platform `metadata_<platform>.json` files in optimized metadata mode, which writes no unified file. A re-render reuses whichever of them exists, so both reuse branches refresh a stale pillar — and neither erases one, since a run whose state has no pillar may simply have lost `pipeline_state.json`. `data.json` carries the *scraped* pillar, so consulting it would file an overridden run under an arm it never used — a confidently wrong label, which for an arm-comparison registry is worse than an empty one.
