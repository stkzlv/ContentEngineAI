...
- **`BaseProductData.to_dict()` is not what writes `data.json` for a scraped product.** The scraper serialises through `_product_to_dict` in `src/scraper/amazon/scraper.py`, a separate hand-written dict, so a field added to the dataclass and to `to_dict()` still never reaches a scraped `data.json` — `pillar` is in both and absent from every scraped file on disk. The topic path does write through `to_dict()`, so a field is persisted there and not for products, which is the confusing half. Check which serialiser your path uses before assuming a new field round-trips.
