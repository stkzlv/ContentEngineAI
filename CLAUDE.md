...
- **The registry CSV header is derived from `fields(RegistryEntry)`.** It used to be a literal list, and `csv.DictWriter` raises on a key its header does not name, so adding a field failed the whole write rather than dropping a column. Don't restate it.
