...
- **`BaseProductData.to_dict()` is the serialiser for everything that goes through `_save_products`.** The topic path calls it directly and the scraper's `_product_to_dict` now delegates to it, so a field added to the dataclass reaches the file on both, and `tests/scraper/test_data_json_serialiser.py` fails if a declared field stops being serialised. **It is not the only writer.** The Botasaurus `@browser` decorator calls `write_scraped_data_output` with the raw extractor dict (`browser_functions.py` `output=` -> `botasaurus_output.py`), and on the `--product-ids` path `BatchController` never calls `_save_products`, so that raw dict is the final `data.json` — missing `rating`, `shortened_affiliate_link` and `pillar`, with `downloaded_images: []` because the callback fires before the media downloads. Check which of the two wrote the file your path produces before assuming a new field round-trips. It compares `to_dict()` against `dataclasses.fields()` rather than against a second dict, because two serialisers can agree with each other and both be missing a field — which is exactly how `pillar` used to persist for topics and vanish for scraped products. The `pillar` *value* still does not reach a scraped file: both assignment sites run after `_save_products`, so every scraper path writes `pillar: null` (#239). `to_dict()` routes both enums through `_enum_value`, since `ProductData(**loaded_json)` does no coercion and a record read back from disk carries plain strings where `.value` would raise.
