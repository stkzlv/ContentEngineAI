...
- **`BaseProductData.to_dict()` is the serialiser for everything that goes through `_save_products`.** The topic path calls it directly and the scraper's `_product_to_dict` now delegates to it, so a field added to the dataclass reaches the file on both, and `tests/scraper/test_data_json_serialiser.py` fails if a declared field stops being serialised. **It is not the only writer.** The Botasaurus `@browser` decorator calls `write_scraped_data_output` with the raw extractor dict (`browser_functions.py` `output=` -> `botasaurus_output.py`). In the standalone CLI's **batch mode** — `--product-ids`, *or* two or more `--keywords` (`scraper.py:1760`) — `BatchController` never calls `_save_products` at all, so that raw dict is the final `data.json`: it carries none of the eight fields above (`rating` and `shortened_affiliate_link` are the ones that would have held values), and `downloaded_images` is `[]` because the callback fires before the media downloads. Only two paths reach `_save_products`: the global batch, via `process_raw_products` (`scraper.py:956`), and a standalone **single**-keyword run, via `scrape_products` (`scraper.py:998`). Check which of the two writers produced the file your path makes before assuming a new field round-trips. It compares `to_dict()` against `dataclasses.fields()` rather than against a second dict, because two serialisers can agree with each other and both be missing a field — which is exactly how `pillar` used to persist for topics and vanish for scraped products. The `pillar` *value* never reaches a scraped file, for two different reasons (#239). On the global batch the assignment (`global_batch.py:1005`) runs after `process_raw_products` has already written the file, so the key is present and null. In CLI batch mode `_save_products` never runs, so the raw dict has no `pillar` key at all, and `batch_controller.py:250` assigns to records nothing then saves. `to_dict()` routes both enums through `_enum_value`, since `ProductData(**loaded_json)` does no coercion and a record read back from disk carries plain strings where `.value` would raise.
...
- **Registry pillar source is `pipeline_state.json`, not `data.json`**: `_read_pillar_from_state` in `product_registry.py` reads from the producer's state file because that's what the producer actually used. `data.json` does NOT carry a pillar value on any path (#239, #241), so the state file is the only source anyway; independently of that, the registry wants the pillar the video was actually rendered under, where a CLI override wins over the product-level one.
