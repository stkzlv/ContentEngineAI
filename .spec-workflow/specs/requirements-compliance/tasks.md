...
- [ ] 1. Create compliance test directory structure
  ...
  - Purpose: Organize requirements compliance tests separately from other tests
  - _Leverage: Existing `tests/` directory structure_
...
- [ ] 2. Create configuration compliance test file
  ...
  - Purpose: Verify configuration system works as documented
  - _Leverage: `src/config_manager.py::UnifiedConfigManager`, existing test patterns from `tests/test_unified_config_system.py`_
...
- [ ] 3. Add secret isolation tests
  ...
  - _Leverage: `config/*.yaml` files, Path operations_
  - _Requirements: 1.4_
...
- [ ] 4. Add configuration validation tests
  ...
  - Purpose: Ensure validation catches errors early with helpful messages
  - _Leverage: `src/video/video_config.py::VideoConfig` Pydantic model_
  - _Requirements: 12.1, 12.2_
  - _Prompt: Implement the task for spec requirements-compliance: First run spec-status, then add validation tests. Role: QA Engineer specializing in validation and error handling | Task: Add tests to test_config_compliance.py that verify Pydantic validation catches invalid configurations and provides clear error messages covering requirements 12.1-12.2 | Restrictions: Must test multiple validation scenarios (type errors, range errors, missing fields), verify error messages contain field names and expected values | Leverage: src/video/video_config.py::VideoConfig Pydantic validators, pytest.raises() for exception testing | Success: Tests verify validation fails for invalid inputs, error messages are descriptive (contain field name and valid range), tests cover type mismatches and constraint violations | Instructions: Mark `[-]` in progress, `[x]` when done_
...
- [ ] 5. Create scraper compliance test file
  ...
  - Purpose: Ensure scrapers follow architectural requirements
  - _Leverage: `src/scraper/base/models.py::BaseScraper`, `src/scraper/amazon/scraper.py::AmazonScraper`_
...
- [ ] 6. Add product data extraction tests
  ...
  - Purpose: Ensure scraped data meets minimum requirements
  - _Leverage: Mock HTML responses, `src/scraper/base/models.py::BaseProductData`_
  - _Requirements: 2.3_
...
- [ ] 7. Add media storage structure tests
  ...
  - Purpose: Ensure outputs follow documented structure
  - _Leverage: `config/core.yaml` output structure configuration, pathlib_
  - _Requirements: 3.1, 3.5_
  - _Prompt: Implement the task for spec requirements-compliance: First run spec-status, then add storage structure tests. Role: Test Engineer specializing in file system testing | Task: Add tests to test_scraper_compliance.py that verify media files are stored in outputs/<product_id>/ directory structure covering requirements 3.1 and 3.5 | Restrictions: Must use tmp_path fixture for isolated testing, verify directory patterns from config/core.yaml, test cleanup functionality | Leverage: pytest tmp_path fixture, config/core.yaml output_structure settings, pathlib for path operations | Success: Tests verify correct directory structure created, media files in expected locations, cleanup removes unexpected files | Instructions: Update `[-]` when starting, `[x]` when done_
...
- [ ] 8. Create video compliance test file
  ...
  - Purpose: Verify subtitle system works as documented
  - _Leverage: `src/video/unified_subtitle_generator.py`, `src/video/subtitle_positioning.py`_
...
- [ ] 9. Add two-part subtitle system tests
  ...
  - Purpose: Ensure two-part subtitle feature works correctly
  - _Leverage: `src/video/unified_subtitle_generator.py`, mock product data_
...
- [ ] 10. Add profile-specific settings tests
  ...
  - Purpose: Ensure profile system works as documented
  - _Leverage: `src/video/video_config.py::VideoProfile`, `config/video_production.yaml`_
...
- [ ] 11. Add style preset system tests
  ...
  - Purpose: Ensure style presets work as specified
  - _Leverage: `src/video/font_color_manager.py`, preset configuration_
...
- [ ] 12. Add ASS effects formatting tests
  ...
  - Purpose: Ensure ASS effects render correctly without literal text
  - _Leverage: ASS file parsing, effect detection logic_
...
- [ ] 13. Add AI service integration tests
  ...
  - Purpose: Ensure AI integrations work reliably with fallbacks
  - _Leverage: `src/video/tts.py`, `src/video/stt_functions.py`, mocking_
  - _Requirements: 10.1, 10.2, 10.3_
  - _Prompt: Implement the task for spec requirements-compliance: First run spec-status, then add AI integration tests. Role: Integration Test Engineer | Task: Add tests to test_video_compliance.py verifying AI service fallbacks and provider prioritization covering requirements 10.1-10.3 | Restrictions: Must mock external API calls, test fallback chains (Google→Coqui for TTS, Whisper→Google for STT), verify primary provider tried first | Leverage: src/video/tts.py provider logic, src/video/stt_functions.py STT fallbacks, aioresponses for HTTP mocking | Success: Tests verify primary providers tried first, fallbacks work when primary fails, Google Chirp 3 HD prioritized for TTS | Instructions: Update `[-]` in progress, `[x]` when done_
...
- [ ] 14. Add pytest configuration for compliance tests
  ...
  - Purpose: Enable running compliance tests separately from unit tests
  - _Leverage: Existing pytest configuration in pyproject.toml_
  - _Requirements: All_
...
- [ ] 15. Create compliance test documentation
  ...
  - Purpose: Help developers understand and maintain compliance tests
  - _Leverage: TESTING.md for format reference_
  - _Requirements: All_
