...
- [ ] 1. Add video metadata extraction function to media_validator.py
  ...
  - Purpose: Provide comprehensive video metadata for producer pipeline
  - _Leverage: Existing `verify_video_file()` FFprobe patterns (lines 173-393)_
...
- [ ] 2. Enhance verify_video_file() to return metadata
  ...
  - Update return type to `tuple[bool, str, dict[str, Any]]`
  - Purpose: Provide validation result and metadata in single call
  - _Leverage: Existing `verify_video_file()` (lines 173-393), new `extract_video_metadata()` from task 1_
...
- [ ] 3. Update downloader.py to handle video downloads with extended timeout
  ...
  - Ensure chunk streaming works efficiently for large video files
  - Purpose: Reliably download large video files without timeouts
  - _Leverage: Existing `download_file_sync()` (lines 457-565)_
...
- [ ] 4. Validate existing video extraction in media_extractor.py
  ...
  - Verify ASIN matching and product-specific video filtering works
  - Add debug logging for extraction method success rates
  - Purpose: Ensure existing video extraction is production-ready
  - _Leverage: Existing `extract_functional_videos_with_validation()` (lines 362-1166), `is_valid_video_url()` (lines 1261-1286)_
...
- [ ] 5. Update scraper orchestration to integrate video metadata
  ...
  - Ensure `downloaded_videos` field populated with relative paths
  - Add error handling for video processing failures (graceful degradation)
  - Purpose: Complete end-to-end video processing with metadata
  - _Leverage: Existing image processing workflow patterns, enhanced `verify_video_file()` from task 2_
...
- [ ] 6. Add video configuration parameters to scraper.yaml
  ...
  - Add `retry_video_downloads: 2` to download_config section
  - Purpose: Make video processing behavior configurable
  - _Leverage: Existing video_config section (lines 108-128), download_config section (lines 130-148)_
  - _Requirements: 5 (Robust Error Handling)_
...
- [ ] 7. Create unit tests for video metadata extraction
  ...
  - Purpose: Ensure metadata extraction reliability
  - _Leverage: Existing test patterns in tests/scraper/, pytest fixtures_
  - _Requirements: 2 (Video Metadata Extraction)_
...
- [ ] 8. Create integration tests for end-to-end video processing
  ...
  - Verify videos downloaded, validated, metadata extracted, paths in data.json
  - Test graceful degradation when no videos available
  - Purpose: Validate complete video processing pipeline
  - _Leverage: Existing integration test patterns, real scraper execution_
...
- [ ] 9. Update documentation in ARCHITECTURE.md and DEVELOPMENT.md
  ...
  - Update configuration options section with new parameters
  - Add troubleshooting section for video processing
  - Purpose: Enable developers and users to understand video capabilities
...
- [ ] 10. Final validation and cleanup
  ...
  - Code review and cleanup (remove debug code, improve comments)
  - Purpose: Ensure production-ready implementation
  - _Leverage: make lint, make test commands_
  - _Requirements: All_
  - _Prompt: Implement the task for spec scraper-video-detection, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Senior Developer with expertise in code quality and production readiness | Task: Perform final validation and cleanup covering all requirements, running comprehensive tests and code review | Restrictions: Must not break existing functionality, ensure all tests pass (lint, type-check, unit, integration), verify error handling in all code paths | Success: All tests pass, code quality checks pass (lint, type-check), tested with 10+ diverse ASINs, graceful degradation verified, code clean and production-ready | Instructions: First edit tasks.md to mark this task as in-progress [-], run all checks and tests, fix any issues, perform code review, then mark as complete [x] in tasks.md_
