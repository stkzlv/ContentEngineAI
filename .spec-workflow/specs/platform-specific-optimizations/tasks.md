...
- [x] 1. Create platform metadata data models
  ...
  - Purpose: Establish type-safe data structures for platform metadata
  - _Leverage: `src/video/config/core_models.py` (Pydantic patterns), `src/scraper/base/models.py` (dataclass patterns)_
...
- [x] 2. Extend DescriptionSettings configuration model
  ...
  - Maintain backward compatibility with existing unified mode
  - Purpose: Integrate platform metadata configuration into existing config system
  - _Leverage: Existing `DescriptionSettings` class, `LLMSettings` patterns_
...
- [x] 3. Create YAML configuration for platform metadata
  ...
  - Add comments documenting each platform's best practices
  - Purpose: Provide production-ready default configuration for all platforms
  - _Leverage: Existing `ai_services.yaml` structure, `video_production.yaml` commenting style_
  - _Requirements: Req 3 (Character Limit Enforcement), Req 4 (Configurable Platform Targeting)_
  - _Prompt: Implement the task for spec platform-specific-optimizations, first run spec-workflow-guide to get the workflow guide then implement the task: Role: DevOps Engineer specializing in YAML configuration and documentation | Task: Create comprehensive platform metadata configuration section in ai_services.yaml following Requirements 3 and 4, documenting all platform-specific settings with inline comments explaining best practices, using existing YAML structure patterns from config/ files | Restrictions: Must maintain YAML file structure consistency, use proper indentation (2 spaces), add comprehensive comments for all settings, provide production-ready defaults based on 2025 research | _Leverage: Follow video_production.yaml commenting style, organize settings hierarchically per platform_ | Success: YAML configuration is well-documented and valid, all platform settings have sensible defaults matching design document, inline comments explain character limits and best practices, configuration loads without errors | Instructions: 1) Mark task in-progress [-], 2) Add configuration with detailed comments, 3) Log implementation with filesModified only (no artifacts needed for config), 4) Mark complete [x]_
...
- [x] 4. Create abstract base generator interface
  ...
  - Add shared utility methods: `_format_prompt()`, `_load_template()`
  - Purpose: Establish common interface for all platform generators
  - _Leverage: `src/scraper/base/models.py` (abstract base patterns), `src/ai/description_generator.py` (LLM utilities)_
...
- [x] 5. Create shared LLM integration utilities module
  ...
  - Add `validate_and_save()` helper for metadata persistence
  - Purpose: Reduce code duplication across platform generators
  - _Leverage: `src/ai/description_generator.py`, `src/ai/script_generator.py` (LLM patterns)_
  - _Requirements: Req 5 (LLM-Powered Generation), Req 7 (Validation)_
  - _Prompt: Implement the task for spec platform-specific-optimizations, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Python Developer with expertise in module organization and code reuse | Task: Create shared utilities module following Requirements 5 and 7, centralizing LLM integration and validation logic to reduce duplication across platform generators, importing and wrapping patterns from description_generator.py and script_generator.py | Restrictions: Must not duplicate existing code, use imports and wrappers only, maintain async/await pattern, do not add platform-specific logic to shared utilities, ensure type hints on all functions | _Leverage: Import from description_generator.py, create thin wrappers, use TypeVar for generic type safety_ | Success: Shared utilities are well-organized in __init__.py, no code duplication from existing generators, all functions are properly typed and documented, platform generators can use these helpers cleanly | Instructions: 1) Mark in-progress [-], 2) Create utility wrappers and imports, 3) Log with artifacts (functions: generate_with_llm, validate_and_save with signatures), 4) Mark complete [x]_
...
- [x] 6. Implement YouTube metadata generator
  ...
  - Implement `validate()` method: check character limits, hashtag count, required #Shorts tag for vertical videos
  - Purpose: Generate YouTube-optimized metadata with SEO keyword placement
  - _Leverage: `BasePlatformMetadataGenerator`, shared LLM utilities, `YouTubePlatformSettings`_
...
- [x] 7. Implement TikTok metadata generator
  ...
  - Implement `validate()` method: check caption length (optimal vs max), hashtag count, blacklist generic tags (#fyp, #foryoupage, #viral)
  - Purpose: Generate TikTok-optimized captions with search keywords and niche hashtags
  - _Leverage: `BasePlatformMetadataGenerator`, shared LLM utilities, `TikTokPlatformSettings`_
...
- [x] 8. Implement Instagram metadata generator
  ...
  - Implement `validate()` method: check caption style compliance, hashtag count (15-30), hashtags in caption not comments
  - Add `_determine_caption_style()` helper method based on `InstagramPlatformSettings.caption_style`
  - Purpose: Generate Instagram Reels captions with extensive hashtag strategy
  - _Leverage: `BasePlatformMetadataGenerator`, shared LLM utilities, `InstagramPlatformSettings`_
  - _Requirements: Req 1 (Multi-Platform), Req 2 (SEO Optimization - Instagram), Req 3 (Character Limits), Req 7 (Validation)_
  - _Prompt: Implement the task for spec platform-specific-optimizations, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Python Developer with expertise in content generation and multi-style outputs | Task: Implement InstagramMetadataGenerator following Requirements 1, 2 (Instagram optimization), 3, and 7, generating captions in either ultra-short (3-5 words) or SEO-descriptive (100-200 chars) style based on settings, with 15-30 hashtags in caption (not comments), leveraging BasePlatformMetadataGenerator and shared utilities | Restrictions: Must extend BasePlatformMetadataGenerator, implement _determine_caption_style() to select between short/SEO based on InstagramPlatformSettings.caption_style, validate 15-30 hashtag range, ensure hashtags embedded in caption text, support emoji if enabled in settings | _Leverage: Use generate_with_llm() with style-specific prompts, validate hashtag count range, check caption_style setting_ | Success: Generator produces valid Instagram captions in both styles, short captions are 3-5 words, SEO captions are 100-200 chars, 15-30 hashtags always in caption, validation enforces count range, caption style matches settings | Instructions: 1) Mark in-progress [-], 2) Implement dual-style generator, 3) Log with artifacts (classes: InstagramMetadataGenerator with generate/validate/_determine_caption_style methods), 4) Mark complete [x]_
...
- [x] 9. Implement platform metadata factory
  ...
  - Map platform strings to generator classes: {"youtube": YouTubeMetadataGenerator, "tiktok": TikTokMetadataGenerator, "instagram": InstagramMetadataGenerator}
  - Purpose: Provide runtime platform selection and multi-platform parallel generation
  - _Leverage: All three platform generators, `asyncio.gather()`_
...
- [x] 10. Modify pipeline step_generate_description for platform awareness
  ...
  - Save metadata to multiple files: `metadata_youtube.json`, `metadata_tiktok.json`, `metadata_instagram.json`
  - If disabled or failure: fallback to existing unified `generate_ai_description()` flow
  - Update `_load_artifacts_generate_description()` to load platform metadata files if they exist
  - Purpose: Integrate platform metadata generation into video production pipeline
  - _Leverage: Existing `step_generate_description()`, `PlatformMetadataFactory`, `generate_ai_description()`_
  - _Requirements: Req 1 (Multi-Platform), Req 4 (Platform Targeting), Req 7 (Validation), Non-Functional (Reliability - fallback)_
  - _Prompt: Implement the task for spec platform-specific-optimizations, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Pipeline Engineer with expertise in async workflows and error handling | Task: Modify step_generate_description following Requirements 1, 4, 7 and non-functional reliability requirement, adding platform metadata generation with factory pattern while maintaining backward compatibility and fallback to unified description generator on failures, updating artifact loading to handle multiple metadata files | Restrictions: Must maintain existing unified flow as fallback, use try-except for graceful degradation, check platform_metadata enabled before factory call, save metadata files with proper naming (metadata_{platform}.json), preserve existing intermediate_paths usage, ensure no breaking changes to pipeline interface | _Leverage: Import PlatformMetadataFactory, use existing save/load patterns, wrap new code in if platform_metadata check, maintain ctx.description for compatibility_ | Success: Pipeline step checks platform_metadata settings, generates platform-specific metadata when enabled, saves multiple JSON files correctly, falls back to unified description on errors, artifact loading works with both old and new formats, no breaking changes to pipeline | Instructions: 1) Mark in-progress [-], 2) Modify step with fallback logic, 3) Log with artifacts (functions: step_generate_description modified, _load_artifacts_generate_description modified), 4) Mark complete [x]_
...
- [x] 11. Create YouTube metadata prompt template
  ...
  - Add examples of high-quality YouTube titles and descriptions
...
- [x] 12. Create TikTok caption prompt template
  ...
  - Add placeholders: {FULL_PRODUCT_NAME}, {PRODUCT_DESCRIPTION}, {PRODUCT_URL}
  - Provide examples of search-optimized TikTok captions
...
- [x] 13. Create Instagram caption prompt template
  ...
  - Add style parameter placeholder: {CAPTION_STYLE} ("short" or "seo")
  - Add placeholders: {FULL_PRODUCT_NAME}, {PRODUCT_DESCRIPTION}, {PRODUCT_URL}
  - Provide examples for both caption styles
...
- [x] 14. Add CLI argument for target platform selection
  ...
  - Add help text explaining platform-specific metadata generation
  - Purpose: Enable runtime platform selection via command line
  - _Leverage: Existing CLI argument patterns in `cli.py`, `argparse`_
  - _Requirements: Req 4 (Configurable Platform Targeting)_
  - _Prompt: Implement the task for spec platform-specific-optimizations, first run spec-workflow-guide to get the workflow guide then implement the task: Role: CLI Developer with expertise in argparse and user experience | Task: Add --target-platform CLI argument following Requirement 4, providing choices for youtube/tiktok/instagram/multi with clear help text, overriding YAML configuration when specified, leveraging existing CLI patterns from cli.py | Restrictions: Must use argparse choices constraint, add to existing argument parser group logically, override config after parsing (not before), maintain backward compatibility (make argument optional with no default to preserve config value), add clear help text explaining functionality and examples | _Leverage: Follow existing --profile, --debug argument patterns, add to appropriate argument group, use config override pattern_ | Success: CLI argument accepts valid platform choices only, help text is clear and includes examples, argument overrides YAML config when provided, backward compatible (no default preserves config), invalid platforms show error with valid choices | Instructions: 1) Mark in-progress [-], 2) Add argument with validation, 3) Log with artifacts (functions: CLI argument parser modified with new --target-platform argument), 4) Mark complete [x]_
...
- [x] 15. Create unit tests for data models
  ...
  - Test settings model defaults and field constraints
  - Purpose: Ensure data models validate correctly and handle edge cases
  - _Leverage: `tests/test_video_config.py` (Pydantic testing patterns), `pytest`_
...
- [x] 16. Create unit tests for platform generators
  ...
  - Test platform-specific rules (e.g., #Shorts for YouTube, blacklist for TikTok)
  - Purpose: Ensure generators produce valid metadata and handle errors gracefully
  - _Leverage: `tests/test_ai_description_generator.py` (LLM mocking patterns), `pytest-asyncio`, `aioresponses`_
...
- [x] 17. Create integration test for pipeline step modification
  ...
  - Test artifact loading from previous runs
  - Test CLI argument override (--target-platform)
  - Purpose: Verify complete pipeline integration and backward compatibility
  - _Leverage: `tests/test_producer_cleanup.py` (pipeline testing patterns), `pytest-asyncio`_
...
- [x] 18. Create validation test for prompt templates
  ...
  - Validate template instructions are complete and clear
  - Verify examples are included in templates
  - Purpose: Ensure prompt templates are well-formed and will produce valid LLM requests
  - _Leverage: `tests/test_ai_script_generator.py` (template testing patterns)_
  - _Requirements: Req 5 (LLM Integration with prompts)_
  - _Prompt: Implement the task for spec platform-specific-optimizations, first run spec-workflow-guide to get the workflow guide then implement the task: Role: QA Engineer specializing in template validation and integration testing | Task: Create validation tests for all three platform prompt templates following Requirement 5, verifying placeholder presence, template formatting with sample data, instruction completeness, and example inclusion, leveraging template testing patterns from test_ai_script_generator.py | Restrictions: Must load actual template files (not mocked), test with real ProductData fixtures, verify all required placeholders exist, validate template structure and formatting, ensure templates produce parseable output when formatted, no LLM API calls (template validation only) | _Leverage: Follow test_ai_script_generator.py template loading patterns, use ProductData fixtures, test format_prompt() output_ | Success: All prompt templates load correctly, required placeholders are present ({FULL_PRODUCT_NAME}, {PRODUCT_DESCRIPTION}, {PRODUCT_URL}), templates format correctly with sample data, instructions are complete, examples are included, templates will produce valid LLM requests | Instructions: 1) Mark in-progress [-], 2) Write template validation tests, 3) Log with artifacts (none for tests), 4) Mark complete [x]_
...
- [x] 19. Update CONFIGURATION.md with platform metadata settings
  ...
  - Provide configuration examples for each platform
  - Document CLI `--target-platform` argument usage
  - Purpose: Help users understand and configure platform-specific metadata generation
  - _Leverage: Existing `CONFIGURATION.md` structure and style_
...
- [x] 20. Update README.md with platform-specific features
  ...
  - Add to feature highlights with supported platforms (YouTube, TikTok, Instagram)
  - Purpose: Inform users about new platform optimization capabilities
  - _Leverage: Existing `README.md` features section_
  - _Requirements: All (user-facing feature announcement)_
  - _Prompt: Implement the task for spec platform-specific-optimizations, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Technical Writer specializing in README documentation and feature communication | Task: Update README.md to announce platform-specific metadata generation feature covering all requirements, adding to features list, providing quick start CLI examples, highlighting supported platforms (YouTube/TikTok/Instagram), following existing README structure and tone | Restrictions: Must maintain README's concise, compelling style, add to appropriate features section, provide copy-pasteable CLI examples, keep explanations brief but clear, don't duplicate detailed docs (reference CONFIGURATION.md), maintain existing formatting | _Leverage: Follow existing README features section format, use code blocks for CLI examples, add to table of contents if needed_ | Success: README clearly communicates new platform optimization feature, CLI examples are practical and copy-pasteable, supported platforms are listed, feature fits naturally into existing README flow, users can quickly understand and try the feature | Instructions: 1) Mark in-progress [-], 2) Update README with feature highlights, 3) Log with filesModified (no artifacts), 4) Mark complete [x]_
