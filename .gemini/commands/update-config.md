...
ls src/video/config/*.py  # Pydantic models (Video)
ls src/scraper/config_models.py # Pydantic models (Scraper)
ls src/**/config*.py      # Module-specific configs
```

## Configuration Structure
- `config/*.yaml` - Runtime configuration (YAML files).
- `src/video/config/` - Video pipeline Pydantic models.
- `src/scraper/config_models.py` - Scraper Pydantic models.
