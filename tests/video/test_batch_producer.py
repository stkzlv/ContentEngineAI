...
from src.video.producer.utils import ProfileUsageTracker, select_profile_for_product, load_profile_pool
...
def mock_outputs_dir(temp_dir):
    ...
    p1 = outputs / "B0VALID1"
    p1.mkdir()
    (p1 / "data.json").write_text(json.dumps({"asin": "B0VALID1", "title": "Product 1", "price": "$10", "url": "http", "platform": "amazon"}))
    
    # Product 2: Valid
    p2 = outputs / "B0VALID2"
    p2.mkdir()
    (p2 / "data.json").write_text(json.dumps({"asin": "B0VALID2", "title": "Product 2", "price": "$20", "url": "http", "platform": "amazon"}))
...
def test_select_profile_for_product_determinism(mock_config):
    ...
    choices = [select_profile_for_product(f"PROD_{i}", pool, mock_config) for i in range(10)]
...
async def test_batch_loop_scenarios(mock_outputs_dir, mock_config):
    ...
    for attr in [
        "subtitle_format", "ass_karaoke", "ass_fade", "preset",
        "subtitle_anchor", "subtitle_margin", "subtitle_content_aware",
        "font_size_scale", "max_subtitle_width_fraction", "subtitle_alignment",
        "max_line_length", "max_words_per_line", "max_duration", "min_duration",
        "subtitle_randomize_fonts", "subtitle_randomize_colors",
        "subtitle_randomize_effects", "subtitle_font", "subtitle_font_color",
        "subtitle_outline_color", "subtitle_background_color",
        "image_width_percent", "image_top_position_percent",
        "target_platform", "metadata_mode"
    ]:
        setattr(mock_args, attr, None if "percent" in attr or "scale" in attr or "margin" in attr or "fraction" in attr or "duration" in attr or "length" in attr or "words" in attr else False)
    ...
    with patch("argparse.ArgumentParser.parse_args", return_value=mock_args):
        with patch("src.video.producer.cli.load_video_config_modular", return_value=mock_config):
            with patch("src.video.producer.cli.setup_logging", return_value=Path("test.log")):
                with patch("src.video.producer.cli.validate_config_and_exit_on_error"):
                    with patch("src.video.producer.cli.load_dotenv"):
                        with patch("os.getenv", return_value="dummy_key"):
                            with patch("shutil.which", return_value="/usr/bin/ffmpeg"):
                                with patch("src.video.producer.cli.create_video_for_product", new_callable=AsyncMock) as mock_create:
