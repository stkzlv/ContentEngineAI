    ...
    def test_path_outside_outputs(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            with patch(
                ...
                external_path = Path("/tmp/some/other/path")
                result = get_relative_path_from_outputs(external_path)
                assert result == "/tmp/some/other/path"
    ...
    def test_all_functions_support_custom_dir(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            with patch(
                ...
                # Product directories
                assert (
                    get_product_directory("B0TEST", custom_name).parent.name
                    == custom_name
                )
                assert (
                    get_product_images_directory("B0TEST", custom_name)
                    .parent.parent.name
                    == custom_name
                )
