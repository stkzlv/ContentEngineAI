...
from src.utils.circuit_breaker import freesound_circuit_breaker, CircuitBreakerError
...
async def test_oauth2_token_refresh_success(fs_client, mock_aioresponses):
    mock_aioresponses.post(
        ...
        payload={"access_token": "new_access_token", "expires_in": 3600, "refresh_token": "new_refresh_token"},
        status=200
    )
    ...
    async with aiohttp.ClientSession() as session:
        ...
        assert fs_client.oauth_access_token == "new_access_token"
        assert fs_client.oauth_refresh_token == "new_refresh_token"
...
async def test_get_valid_oauth2_token(fs_client, mock_aioresponses):
    ...
    fs_client.oauth_access_token = "valid_at"
...
@pytest.mark.asyncio
async def test_download_full_sound_oauth2_success(fs_client, mock_aioresponses, temp_dir):
    mock_aioresponses.post(
        "https://freesound.org/apiv2/oauth2/access_token/",
    ...
    with patch.object(fs_client.fs_api_client, "get_sound") as mock_get_sound:
        mock_get_sound.return_value = {"name": "Test Sound", "username": "user", "license": "CC0", "url": "http://fs.org/s/456/"}
        
        async with aiohttp.ClientSession() as session:
            result = await fs_client.download_full_sound_oauth2(sound_id, temp_dir, session)
...
@pytest.mark.asyncio
async def test_download_sound_preview_with_api_key_success(fs_client, mock_aioresponses, temp_dir):
    ...
    mock_aioresponses.get(
        "https://fs.org/preview.mp3",
        body=b"preview data",
        status=200
    )
    
    async with aiohttp.ClientSession() as session:
        result = await fs_client.download_sound_preview_with_api_key(mock_sound, temp_dir, session)
        assert result is not None
        path, attribution = result
        assert "Preview_Sound.mp3" == path.name # Sanitized name
...
@pytest.mark.asyncio
async def test_token_persistence_fail(fs_client, mock_aioresponses, temp_dir, monkeypatch):
