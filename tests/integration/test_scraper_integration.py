...
def test_full_scrape_workflow(scraper, mock_outputs_root):
    ...
    with patch("src.scraper.amazon.scraper.create_dynamic_browser_function") as mock_create_func, \
...
def test_scrape_graceful_failure(scraper, mock_outputs_root):
    ...
    with patch("src.scraper.amazon.scraper.create_dynamic_browser_function") as mock_create:
        # Mock browser failure
        with patch.object(scraper, "_scrape_with_retry", side_effect=Exception("Browser crashed")):
