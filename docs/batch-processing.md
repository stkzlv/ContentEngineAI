...
Topics are isolated: one failing does not abort the rest, and the closing summary names the step each failure stopped at. Each result is checked with `ffprobe`, not by exit code, because an exit code cannot tell this run's render from an earlier profile's, and a clean exit is not the same as a readable file. A timeout no longer leaves a truncated `.mp4` under the finished render's name: the assembler writes to a temporary file and renames it only after FFmpeg exits cleanly.
