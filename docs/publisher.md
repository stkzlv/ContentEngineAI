...
- **Creates a single post per product for all platforms**
  - Example: 10 products × 3 platforms = 10 posts total (one per product)
  - Unified metadata is loaded once and published to all platforms in a single API call
...
The publisher creates **one post per product** published to all platforms simultaneously. Both the CLI (`single` command) and global batch pipeline use a single API call with all platforms, resulting in one shared `post_id`.
...
**Scheduling**: All platforms for the same product share the same time slot and post ID. Example: 3 products → 3 scheduled posts (one per product, each targeting 3 platforms).
