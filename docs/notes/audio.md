...
- **Jamendo answers an identical query inconsistently.** Six repeats of one `fuzzytags=lofi` request returned `5 0 0 5 5 0`, and the pattern holds with and without `durationbetween`, `vocalinstrumental`, `audiodlformat` and `include`, so it is the service, not a parameter. The provider drew a fresh random query per attempt and treated one empty answer as final, so three attempts were three coin flips and roughly one render in eight fell through to Freesound previews. A zero result is a reason to repeat the same query, not to move on (#516).
