...
| TikTok AI-generated-content label (`videoMadeWithAi`) | Shipped, **on by default** | TikTok platform policy. Configurable via `config/publisher.yaml::tiktok_settings.video_made_with_ai`; **on by default**. **Correction pending (#558):** TikTok's 2026-H2 guidelines, effective 24 September 2026, say disclosure is not needed for "generic text-to-speech (TTS) narration, when the TTS isn't a recognizable voice of a known individual" (https://www.tiktok.com/community-guidelines/en/integrity-authenticity); a label is required for AI or edits showing realistic people or scenes, or audio mimicking a real person. Every render here uses a generic TTS voice over real photos and stock footage, so the label is not required by that text. It stays on until #558 records whether to keep it voluntarily. Disclosing is also the cheaper error: TikTok reads C2PA credentials and auto-labels undisclosed AI content, and enforcement escalates from a warning to a posting restriction to a ban. The label is not free: viewers can turn AI content down, and a large study found disclosure cut engagement by 7-8% (`docs/creator-research.md` section 1), and it is not required for generic TTS; #558 decides whether to keep it voluntarily. Sent flat beside `tiktokSettings`, which is where the SDK models it. The opt-out reaches every publish path, including the global batch, which builds its own publisher from the same loaded config the publisher CLI uses. |
