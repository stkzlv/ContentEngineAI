...
| Caption text contains `#ad` | Existing in metadata templates | Partial FTC compliance. Phase 0.2 makes the disclosure the first line of the caption, before any other text or hashtags. |
...
| First-line caption disclosure on every platform | **(planned)** Phase 0.2 | FTC two-punch (caption component) + Spain CNMC. Disclosure leads the caption text, language-matched to the script. |
...
| Localized disclosure variants | **(planned)** Phase 0.4 | FTC same-language rule + Spain Royal Decree 444/2024. Spanish renders emit `#publi` or `#publicidad`; English renders emit `#ad`. |
