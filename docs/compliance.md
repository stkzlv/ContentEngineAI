...
| First-line caption disclosure (`#ad` leads the caption on TikTok / Instagram / YouTube) | Shipped | FTC two-punch (caption component) and Spain CNMC. Configurable per render via the `disclosure` field on `PublishMetadata`; defaults to `#ad`. Both this and the overlay are conditional on the render carrying a material connection: a topic render with no affiliate link emits neither, since a disclosure on content with no such connection is a false statement about one and costs reach besides. The gate defaults to disclosing, so only a record that positively shows there is nothing to disclose suppresses it; a product whose affiliate link failed to build still discloses. The producer records the decision in `metadata.json` and the publisher reads it, rather than the two deriving it separately and disagreeing. The string-only escape hatch lets language-aware variants inject `#publi` for Spanish renders without further code change. |
