...
| TikTok AI-generated-content label (`videoMadeWithAi`) | Shipped, **on by default** | TikTok platform policy. Configurable via `config/publisher.yaml::tiktok_settings.video_made_with_ai`; **on by default**, the opposite of the YouTube row above, because the two platforms draw the line in different places and this pipeline lands on opposite sides of it. TikTok requires the label for AI-generated speech and extends it to AI voiceover even when the footage is real; every render here carries an AI TTS voiceover. Script writing, captions and hashtags are exempt, but the voice is not. Disclosing is also the cheaper error: TikTok reads C2PA credentials and auto-labels undisclosed AI content, and enforcement escalates from a warning to a posting restriction to a ban. The label is not free: viewers can turn AI content down, and a large study found disclosure cut engagement by 7-8% (`docs/creator-research.md` section 1), but it is required. Sent flat beside `tiktokSettings`, which is where the SDK models it. The opt-out reaches every publish path, including the global batch, which builds its own publisher from the same loaded config the publisher CLI uses. |
