...
Trust the artifact over the exit code. The global batch now exits non-zero when no product completes end-to-end, but a partial failure exits 0 unless `--strict` is passed. Confirm success by grepping the phase-summary log lines (`Scraping phase complete:`, `Production phase complete:`, `Publishing phase complete:`), not `$?`.
