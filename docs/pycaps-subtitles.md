...
- **Single-line captions only.** Two-part (upper URL + lower voiceover) is
  FFmpeg-only. When a profile has `two_part_subtitles_enabled: true` (now
  `two_part_subtitles.enabled: true` under the nested block) and you flip the
  engine to pycaps, the producer logs a warning and disables the two-part
  system for that run. The upper URL is not rendered.
