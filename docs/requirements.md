...
- Every script, product or topic, ends on exactly one configured call to action, verbatim, as its final sentence. The options live in `script_templates.cta_options` and `cta_options_topic` and are rendered into each template's rules immediately after the closing-beat rule, because an instruction stated once in the narrator profile did not bind. Script validation refuses a script whose last sentence is not one of them and re-enters the retry loop; if every attempt misses, the first otherwise-complete script has the first option appended, with a warning, rather than the render being lost.
