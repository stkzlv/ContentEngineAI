...
- The configured `default_search_parameters` (price, rating, prime) apply to every entry point, the standalone CLI and the global batch alike, unless a CLI flag overrides a field; the search log line states the filters in force
...
- **Audio handling**: source video audio is dropped; the mix is voiceover and background music, plus an optional signature sting at the start or the end, mastered with the rest
...
- Pycaps default position: a deliberate lower-third (block bottom at 75% of the frame), below the 2026 safe-zone bottom (65%). The image profiles follow the block -- the assembler fits a product image above it wherever it is -- so the remaining reason for the low position is the centred source video on the `product_video_*` profiles, which ends near 66%. Template's own alignment preserved unless explicitly overridden.
...
- Topic renders use their own narrator profile. The default one is written for someone describing a purchase, and the product call-to-action list implies something to buy, so a topic script would otherwise close by pointing at something to buy that does not exist
...
- Every **product** template instructs the LLM to end the spoken script with one short closing question or claim, right before the CTA, not replacing it. Personal and storytelling templates use a two-option opinion question (comment-fork); analytical and comparison templates use a debatable but defensible spec claim that invites a correction. The closing line invites conversation; comments are a weak ranking signal on Shorts, and asks for a specific word, emoji, share or tag are demoted as engagement bait and excluded.
- Every script, product or topic, ends on exactly one configured call to action, verbatim, as its final sentence. The options live in `script_templates.cta_options` and `cta_options_topic` and are rendered into each template's rules -- immediately after the closing-beat rule on product templates, last in the Rules block on topic templates -- because an instruction stated once in the narrator profile did not bind. Script validation refuses a script whose last sentence is not one of them and re-enters the retry loop; if every attempt misses, the first otherwise-complete script has the chosen line appended, or substituted for a last sentence that reads as a paraphrased CTA, with a warning, rather than the render being lost. The line is chosen per record by salted hash and only that one is rendered into the rule, because quoting the whole list left the choice to the model and it took the first entry on every render; validation still accepts any configured line.
...
- Scripts can be asked to sound conversational: contractions, spoken fillers, one emotional beat, and at the higher setting one self-correction of wording. Off by default, which leaves the prompt unchanged. Filler never lands in the first sentence, inside a number, name or claim, or in or after the call to action, and uses words the captions will carry rather than sounds speech recognition drops.
- Scripts can carry a recurring author signature: an opener that starts the first sentence, a transition used once where the script turns, and a sign-off spoken right before the call to action. Each is drawn per product below a configured rate, so the signature recurs without appearing in every render. Off by default. The first comment and the platform captions still quote the closing line, not the sign-off.
...
- The per-platform caption generator receives the rendered spoken script and mirrors the script's closing line into the caption body before the hashtag block. Same line in spoken audio + on-screen subtitle + caption text (Rule of 3s). When no script is available, the caption falls back to the platform's standard search-optimised content with no closing line.
