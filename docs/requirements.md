...
- Every template instructs the LLM to open with a natural conversational hook that carries the long-tail audio keyword (product category, problem or benefit, audience or use context, and a price band when supported) embedded inside speech a person would actually say out loud. The keyword must be present in the audio without the opener reading as a search-bar query. Good shapes include problem-first ("Most X under $Y fail at Z..."), direct address ("If you're [audience] and [problem], this $N..."), discovery framing, and myth-tension; the literal "Best [category] under $[N] for [audience]" Google-query shape is an anti-pattern. TikTok transcribes spoken audio and ranks it as a primary search signal, so the keyword's presence in the first 5 seconds of TTS determines whether the video gets discovered.
